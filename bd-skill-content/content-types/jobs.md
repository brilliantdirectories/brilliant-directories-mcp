...
| `post_price` AND `post_promo` | Pass the same numeric value to both. Salary or hourly rate as shown in the source — no currency symbol, no commas, decimals optional. Hourly source → `14.50`; annual source → `70000.00`. Do not convert between hourly and annual. On a salary range, use midpoint of low+high. OMIT both on "commensurate" / "DOE" / "competitive" / missing — never fabricate. |
