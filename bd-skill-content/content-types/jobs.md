...
| `post_start_date` | Role start date when listed in source, format `YYYYMMDD` (8 digits, **NO time** — different from events' 14-digit format). OMIT when source has no start date. Example: `"20260901"`. |
| `post_promo` | Yearly salary, numeric only — no currency symbol, no commas, decimals optional (`75000` or `75000.50` both valid). On salary range, use midpoint of low+high. OMIT on "commensurate" / "DOE" / "competitive" / missing — never fabricate. |
| `post_job` | **Discover at runtime** from cached `getPostTypeCustomFields.post_job.choices` (Step 3). BD default is `Full-Time` / `Part-Time` / `Freelance` / `Internship` / `Consultant` / `Contract`, but admin may have customized — always map source text against the LIVE choices, not the defaults. Map case-insensitive to the closest semantic match in the live enum ("full time/FT" → whichever live choice means full-time; "intern" → whichever means internship; "contract/contractor" → contract-equivalent; etc.). If source text is ambiguous or no live choice plausibly matches, OMIT the field — never guess. |
...
| `post_location` | **Full address with street + city + state/region + zip when available** — same shape as events. Do NOT prepend the company name (already in `post_venue`). Example: `"500 W 2nd St, Austin, TX 78701"`, NOT `"Acme Corp, 500 W 2nd St, Austin, TX 78701"`. Many remote/hybrid postings have no street; OMIT then. |
