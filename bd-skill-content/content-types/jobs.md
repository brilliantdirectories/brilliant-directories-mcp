...
| `post_price` AND `post_promo` | Salary or hourly rate as shown in the source — numeric only, no currency symbol, no commas, decimals optional. Hourly source → `14.50`; annual source → `70000.00`. Do not convert between hourly and annual. On a salary range, use midpoint of low+high. **Pass the same value to BOTH `post_price` and `post_promo`** (BD job theme reads both depending on configuration). OMIT both on "commensurate" / "DOE" / "competitive" / missing — never fabricate. |
