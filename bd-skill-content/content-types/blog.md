...
10. **Image dedup (FEATURE).** Executed as the final step of METHODOLOGY Stage 5 (after orientation gate passes). For blog: `listSingleImagePosts property=original_image_url property_value=<exact URL> property_operator=eq limit=1`. `total > 0` → dupe, pick another. `total == 0` → safe to commit.
