...
7. **Pre-create batch — this turn's only job, every call in ONE message.** 7a, 7b, and 7c need nothing from each other's results — all three fire together in this one single message, never in separate turns: one survivor = six calls — the `poolImages` call, the title check, and the survivor's four `Geocode ladder` tiers; each additional survivor adds its four tiers to the same message. No other calls ride this turn. (METHODOLOGY `Image strategy` Steps 1-3 path: 7a-7b calls follow that sequencing instead — 7c still fires here.)
