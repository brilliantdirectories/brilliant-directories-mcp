...
## Rule: Task instructions

Obey a task's goal choices — scope, sources, ranking, duplicate criteria, content shape, fields, publish status, images, receipt form. Execution mechanics stay this file's own under any instruction set: packs, probe batching, probe timing, and stopping run per their rules. A task's source list is a coverage list — one pass per source, in the task's order, follow-on pages that source links included; a source once passed is spent, and its fetched documents are its answer — a task's "confirmed" or "official" wording asks no further act of a document in hand. An extra verification act a task names — a second independent source — is scope: it runs, packed per **Rule: Search discipline**. A fact that changes on a later read re-probes. Each pool-print fires its probes as one batch; candidates surfacing later form a new pool-print, one batch again. A task's "check duplicates before creating" names the census's own timing — met at the pool-print; its duplicate criteria apply when reading verdicts, and extra duplicate sources a task names ride the census batch. A task's ranking or selection order chooses from the verdict lines. The hunt ends when goal-count unique survivors exist, mid-list included — ranking applies among everything seen by then; a survivor lost after the stop re-opens the hunt.
...
5. **Search round** — one turn of five queries per the `Source candidates` section and METHODOLOGY `Stage 3: Source research` steps 2a-2b. Its score: how many results it surfaces showing a job title — ten candidates beat one. The opening round's message also carries Step 3's `getPostTypeCustomFields` call in its sixth slot.
6. **Pool-print turn — this turn's only job.** Every candidate showing a job title — from a search result or a fetched page — enters ONE pool (the 30-day staleness gate applies per `Source candidates` — a candidate showing no posted-date is never blocked by it): print the numbered pool (best-fit, up to 10) per METHODOLOGY `Candidate pool discipline (universal pattern)`, and this same message fires Step 7's calls for the whole pool — ONE title compound covering every pooled candidate. The pool-print turn's only legal tool is `listSingleImagePosts` — this one call shape, nothing else. Once a candidate shows its title, its research stops until it survives Step 7. Count the title-bearing candidates — every one enters the pool, best-fit 10 when more: N pooled = 1 compound covering all N, in this message — a compound missing any pooled candidate's variants is an incomplete turn. One candidate is a pool of one — it prints as `1.` and its compound fires the same. None to dedup → return to Step 5.
...
8. **Pre-create batch — this turn's only job: call 8a, 8b, and 8c in this ONE message.** One survivor = six calls — its `poolImages` call, its title check, and its four `Geocode ladder` tiers; each additional survivor adds its own six calls to this same message. No other calls ride this turn. The six are three tools' calls — `poolImages`, `listSingleImagePosts`, `WebFetch` — born to fire together.
