...
4. **Menu link inventory — one call:** `listMenuItems limit=100 property=is_default property_value=false property_operator=eq` (follow `next_page` while present) — platform-default rows are excluded server-side; only the site's own customized menu items return. `property_value` must be the literal `false` — `0` is silently ignored on this boolean field. Empty-link rows are already excluded. Cache `{menu_name → menu_link}` as internal-link candidates; skip rows whose `menu_link` contains `%%%`. Zero rows → proceed without menu links.
