...
**Application CTA** — unless the user requests otherwise: when an application or official job information URL is known, place this block right after the intro — a short record-material `<h2>`, a 2-3 sentence third-person `<p>` on how to apply (the steps, what to have ready — never commands, never the button, link, or form described, never what they open), then `<p><a class="btn btn-secondary btn-lg vmargin" href="<verified official URL>" title="<descriptive phrase>" rel="noopener" target="_blank">Button Text In Title Case</a></p>`. The URL is the button's first — never a body citation; never the button alone; never an action or login-gated path — public pages only (a login page only when it is itself the destination).
...
| `post_start_date` | Required. The source's apply-by date — application close date, deadline, listed start date, and similar are all this one date. Else identical to `post_live_date`. `YYYYMMDDHHmmss` (14 digits). A date listed without a clock time → `000000`. |
