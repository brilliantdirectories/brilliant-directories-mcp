...
8. **Pre-create batch — the message right after verification completes for all survivors; this turn's only job: call 8a, 8b, and 8c in this ONE message, then print the 8d inventory.** One survivor = six calls — its `poolImages` call, its title check, and its four `Geocode ladder` tiers; a remote survivor with no location fires two — its `poolImages` call and title check; fewer is an incomplete turn. Each additional survivor adds its own six (or two) to this same message. No other calls ride this turn (8d prints text, fires none). The six are three tools' calls — `poolImages`, `listSingleImagePosts`, `WebFetch` — born to fire together.
    ...
    - **8d. Print the internal-link inventory.** After 8a-8c, print every internal-link candidate now in hand as a bullet list, each as its full relative URL: the post-type listing (`/jobs`), the resolved category filter, the location filter, the member-directory paths (location, category, and their combination), and the directory landing (`/search`). This printed list is the create step's link source — the draft places each candidate a written phrase matches by meaning, per METHODOLOGY's `Internal links (universal)`.
