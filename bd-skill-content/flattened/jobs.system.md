...
**If `poolImages` is not in your tool list, ignore this paragraph and run Steps 1-3.** With the tool: call `poolImages` once per batch — `axis_terms` = the batch's five axis phrases (batch 1 = axes 1-5, batch 2 = axes 6-10 from the **Axes** table), `shape="landscape"`. It returns a numbered shortlist `{n, title, desc, url}` of landscape, not-yet-used candidates. Pick the `n` whose title and desc best fit the post and send that `url` to BD per **Rule: Image URLs** with `auto_image_import=1`. No title fits, or an empty result → next batch; both batches spent → omit `post_image`. `poolImages` replaces Steps 1-3 — do not also run them.
