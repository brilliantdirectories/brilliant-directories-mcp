...
| `post_location` | The display address — full street when the source gives one, else city/state (the string that geocoded, e.g. `"Denver, CO"`); lat/lon are the map coordinates. A multi-city source pins the post to ONE city — the source's primary or first-listed — for the title, location, and geocode; the other cities are body facts only. Do NOT prepend the company name (already in `post_venue`). Remote with no location: OMIT. |
