...
5. **Source discovery.**
    ...
    - **5b. Pool** — every WebSearch result already showing a job title pools as-is (best-fit, up to 10; the 30-day staleness gate applies per `Source candidates` — a result showing no posted-date is never blocked by it). Capture and print the numbered pool per METHODOLOGY `Candidate pool discipline (universal pattern)` — Step 6 fires in that same message; none keyed → straight to 5c.
