...
5. **Search round — the opening round fires exactly six calls in ONE message: five queries per the `Source candidates` section and METHODOLOGY `Stage 3: Source research` steps 2a-2b, plus Step 3's `getPostTypeCustomFields` call; the round's slots are exactly these. Later rounds fire five new-angle queries.** Its score: how many results it surfaces showing a job title — ten candidates beat one.
6. **Pool-print turn — the message right after a search round's results arrive; this turn's only job.** Every candidate showing a job title — from a search result or a fetched page — enters ONE pool (the 30-day staleness gate applies per `Source candidates` — a candidate showing no posted-date is never blocked by it): print the numbered pool (best-fit, up to 10) per METHODOLOGY `Candidate pool discipline (universal pattern)`, each line as `Title — Company — v1 | v2 | v3` (its three title variants, each 1-3 words), and this same message fires Step 7's calls for the whole pool — ONE title compound covering every pooled candidate. A round has ONE pool-print turn: every candidate's calls fire here — a second dedup turn on the same round's results is an incomplete first turn. The pool-print turn's only legal tool is `listSingleImagePosts` — this one call shape, nothing else. Once a candidate shows its title, its research stops until it survives Step 7. Count the title-bearing candidates — every one enters the pool, best-fit 10 when more: N pooled = 1 compound covering all N, in this message — a compound missing any pooled candidate's variants is an incomplete turn. One candidate is a pool of one — it prints as `1.` and its compound fires the same. None to dedup → return to Step 5.
...
8. **Pre-create batch — the message right after verification completes for all survivors; this turn's only job: call 8a, 8b, and 8c in this ONE message.** One survivor = six calls — its `poolImages` call, its title check, and its four `Geocode ladder` tiers; each additional survivor adds its own six calls to this same message. No other calls ride this turn. The six are three tools' calls — `poolImages`, `listSingleImagePosts`, `WebFetch` — born to fire together.
