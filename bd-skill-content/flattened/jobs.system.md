...
5. **Search round** — one turn of ten queries per the `Source candidates` section and METHODOLOGY `Stage 3: Source research` steps 2a-2b. Every query carries a recency term (`hiring now`, `<month year>`) — a bare year returns stale postings. Its score: how many results it surfaces showing a job title — ten candidates beat one.
