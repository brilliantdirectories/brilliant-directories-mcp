...
7. **Pre-create batch — every call in ONE turn.** Steps 7a-7c are independent — their calls fire together in one message: one survivor = six calls — the `poolImages` call, the title check, and the survivor's four `Geocode ladder` tiers; each additional survivor adds its four tiers to the same message. (METHODOLOGY `Image strategy` Steps 1-3 path: 7a-7b calls follow that sequencing instead — 7c still fires here.)
