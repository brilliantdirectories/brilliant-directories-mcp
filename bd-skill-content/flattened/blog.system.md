...
A resolved combo also yields each of its dimensions solo as a separate candidate — the more filters resolved, the more distinct targets in hand. Bank the combo and its solos; each rides a different draft phrase. A phrase naming two dimensions takes the combo; a phrase naming one takes that solo — every distinct matching phrase takes its target this way. Most-specific only breaks ties between URLs for the same phrase, never skips a valid target:
