...
Cached data feeds Stage 4 category routing and the internal-link inventory — the run's hoarded link candidates: the menu links, and, as each is cached, every URL-PATTERNS Pattern 1/2/3/5 target the run's own data yields (post rows dedup returns, the post type's listing and category filters, the directory landing). Pattern 6 holds only its resolved slug ingredients, never an assembled URL, until its gate clears.
...
4. **Internal links riding the draft's own nouns** — use URL-PATTERNS.md Pattern 1 (specific post URLs) or Pattern 3 (the post-type page carrying at least one filter — category, location, date, or combos). **Write the entire body first, with zero links. The hoard informs link placement only, never which facts the draft covers. The linking pass's targets are the run's verified internal targets — live posts this run's dedup/list calls already returned (**`post_status=1`**) and Pattern 3/6 URLs constructed from the post's own category, location, and — for events — date values — then wrap noun phrases the finished draft already contains — no link in the post's first sentence — onto targets from that list — a target is eligible only when a noun the draft already wrote is present: the exact place, name, role, or subject a category plainly covers (its own word, an inflection, or a noun inside its subject matter — "water workout" rides its category); the draft's word rides the anchor, the ledger label rides the URL only, and taxonomy words ("category", "section", "archive") never enter the prose. A Pattern 6 slug uses only the location segments whose place noun the draft wrote and the category segment its subject noun, built per **Link shape priority** — at most one link per sentence, at most two per paragraph, no href twice, no anchor phrase twice, distributed evenly across the post's full length and across the Pattern types the draft's nouns support — never clustered, never one Pattern carrying the page — hitting — never exceeding — the internal-link budget: **at most one internal link per ~125 words, capped at 12 total (a ~500-word post at most 4, a 1,000-word post at most 9, a 1,500-word-or-longer post at most 12) — a ceiling, never a figure to reach. A count under the cap is correct — never a signal to link more nouns; a shortfall is never a defect and never a prompt to add, bend, or hunt for a noun**. The linking pass may not add, reshape, or reorder a single sentence. The chosen targets' gate checks (member-count, taxonomy, and title-filtered post lookups) share ONE message — one check per selected link, no more. This is the run's only gate message: a cleared check keeps its one link — one check, one placement; a failed or unchecked check drops its link — its noun stays unlinked, re-targeted only if the same sentence's noun already fits a run-verified post URL, a Pattern 3 URL in any filter shape, or Pattern 5; otherwise it carries no link.** **A sentence, bullet, or clause that exists for a link is a worse failure than an unlinked noun; wrapping a noun a sentence already owns is never the crime; the budget never outranks the prose.** Anchor text reads as part of its sentence — a sentence about the topic, never about the linked destination — never a standalone CTA, never a trailing "More X in Y" section. Never fabricate URLs; a reference with no verifiable target omits that link.
...
19. Count the internal links (relative `<a href="/...">`). Zero while a draft noun matches a target this run's gate message cleared → the pass was skipped; run it per METHODOLOGY item 4. Zero because no draft noun matched a cleared target → correct; add no noun and bend no sentence.
