...
**If `poolImages` is not in your tool list, ignore this paragraph and run Steps 1-3.** With the tool: call `poolImages` once per batch — `axis_terms` = the batch's five axis phrases (batch 1 = axes 1-5, batch 2 = axes 6-10 from the **Axes** table), `shape="landscape"`. It returns a numbered shortlist `{n, title, desc, url}` that is already orientation-filtered AND deduped against the site — every entry is ready to commit. Your only action: pick the `n` whose title and desc best fit the post and send that `url` to BD per **Rule: Image URLs** with `auto_image_import=1`. Do NOT dimension-check, dedup, or verify the URLs — `poolImages` did all of that; running any `getImageDimensions` or `listSingleImagePosts` dedup call after it is a mistake. No title fits, or an empty result → call `poolImages` again with the next axis batch; both batches spent → omit `post_image`. `poolImages` fully replaces Steps 1-3.
