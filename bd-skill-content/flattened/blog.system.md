...
1. `getSiteInfo` → industry, profession, primary_country, language, timezone (IANA identifier, e.g. `America/Los_Angeles`), `current_site_datetime` (site-local now, `YYYYMMDDHHmmss`), brand, directory landing URL. Bank `/` + `main_directory_url_relative` (never the absolute sibling) into the internal-link inventory as the Pattern 5 candidate — every internal href in a post body starts with `/`.
