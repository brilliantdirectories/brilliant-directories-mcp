...
Fuzzy-match source category vs the **category ledger** list. ≥70% confidence → carry the LEDGER value forward, never the source's wording. <70% → SKIP the record (do NOT create categories). Bank every ledger category into the internal-link inventory as its own Pattern 3 filter candidate. The member top/sub taxonomy is also a Pattern 6 member-directory ingredient marked "pending gate" — any draft phrase that means a member category rides it (matched at the gate from `listTopCategories`/`listSubCategories`, never the ledger value), standalone (`/top`, `/top/sub`) or combined with location; the member-count gate verifies each (count >= 1 to link, per URL-PATTERNS).
