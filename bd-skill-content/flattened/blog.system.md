...
10. **Image selection — FEATURE image only.** On the `poolImages` path nothing fires here — the call fires in the pre-create batch (Step 13), never its own turn. Steps 1-3 image path (no `poolImages` tool): run METHODOLOGY `Stage 5: Content manufacture (universal)` → `Image strategy` end-to-end. Inline body images: see the `Inline body images` section.
