...
Fuzzy-match source category vs the **category ledger** list. ≥70% confidence → carry the LEDGER value forward, never the source's wording. <70% → SKIP the record (do NOT create categories). Bank the resolved category (with any geocoded location) into the internal-link inventory as a Pattern 3 filter candidate, and as a Pattern 6 member-directory ingredient marked "pending gate" — the member-count gate in the pre-create batch assembles and verifies it (count >= 1 to link, per URL-PATTERNS).
