...
**Bank internal-link candidates as you go:** at each step that exposes one, bank the candidate into the internal-link inventory (no new call, no new turn). You are forgetful, so reprint the full inventory — every candidate's bare URL, as a plain list — to yourself in every message from the first candidate until the post is created; a candidate missing from the reprint is lost. Spend the full inventory onto the draft at content-write time.
...
2. `listTopCategories limit=25` → the categories actual site members are assigned to (e.g. "Personal Training", "Group Fitness") — NOT post-type categories. Real sites can have 100s of rows; 25 reads the vertical. Bank each returned category filename now as a Pattern 6 member-directory candidate (`/<filename>`); do not use it for post-category routing — post categories come from the resolved post type's `feature_categories` field (step 3).
...
Cached data feeds Stage 4 category routing and the internal-link inventory — the run's link candidates, each banked and printed the moment the run can form it and kept to the end, the inventory only growing — each URL enters once, a re-encounter adds nothing — spent onto the draft at Stage 5's linking pass, all first-class: the menu links; live posts dedup returned (Pattern 1); the post type's own listing (Pattern 2); its categories, location (from the post's geocoded lat/lng), and — for events — date filters (Pattern 3); the filtered member directory — members by category and/or location, e.g. "acupuncturists in Denver" (Pattern 6); specific member profiles (Pattern 4); and the directory landing (Pattern 5). Pattern 6 and Pattern 4 hold only their resolved ingredients, never an assembled URL, until their gate clears.
...
Run all pool candidates together, in ONE turn — the same turn the pool prints. A candidate at any later point without its verdict line → run Stage 2 now for every verdict-less candidate, before their next call. A dupe drops for the cost of one dedup round, not a wasted research cycle. Never bulk-list a site's existing posts. Print every live (`post_status=1`) row this stage returns into the internal-link inventory as a Pattern 1 candidate — a Pattern 1 candidate, wherever banked, prints `/<post_filename> — match-subject: <its title minus employer, brand, location, and date segments — the remaining role, event, or topic words>` — the dupes' rows included; task scope selects candidates for creation, never rows for banking.
...
Use Pexels for all images. After both attempts yield no commit, omit `post_image`. Omitting is the last resort.
...
**If** `poolImages` **is not in your tool list, ignore this paragraph and run Steps 1-3.** With the tool, `poolImages` replaces Steps 1-3: call it once — `axis_terms` = all ten axis phrases from the **Axes** table, `shape="landscape"`. In a runbook step that batches calls, `poolImages` is born to fire alongside the other tools' calls — it rides that step's batch message. It returns a numbered shortlist `{n, title, desc, url}`, already orientation-filtered and site-deduped. Pick the `n` whose title and desc best fit and put that `url` in the post's create call per **Rule: Image URLs** with `auto_image_import=1`. The image is then settled — do NOT re-check it: no `getImageDimensions` and no `listSingleImagePosts` dedup on a `poolImages` url. No title fits, or an empty result → call `poolImages` again with ten new phrases — each axis aimed at a different scene, never a synonym of its first phrase; both calls spent → omit `post_image`.
...
- The page speaks as the thing it is: it knows the facts, not where they came from — no posting, listing, or source exists in its world to be a sentence's subject.
...
- Every voice: declarative. State what a thing is with the bare verb — "is", "is at", "opens", "costs" — and never swap in a light or transitive verb to make a fact look like an action. Every sentence's subject comes from a closed set — a person, an organization, a place, or the record and its real parts (the venue, the class, the booth — never the posting, listing, page, or source); nothing abstract ever holds the subject slot, under any verb, and the record never acts on people or places. Facts stated plainly, not hedged.
...
- **False agency** — an attribute, setting, or format as benefactor: inanimate subject + causative verb + evaluative payoff ("The seamless course elevates race day"). The payoff is the test, anywhere in the sentence — a trailing ", which ..." clause included: a measurable fact rides any subject ("The Cloverdale coach role pays competitive wages with commissions"); an evaluative or summary-noun payoff never does — state the bare fact or who does what.
- **Evidential subject** — subject is the evidence for a fact (an address, a document, a sign) and the verb is an inference verb (places, points to, suggests, indicates, shows) reaching the real fact? The sentence runs backwards. State the concluded fact with the thing itself as subject ("The studio is located in Wynwood").
- **Fact-gloss** — a sentence or clause re-describing a stated fact as its mood, ease, appeal, or reach ("starts at 8:00 a.m." → "a short runway before the course begins"), or asserting such a quality with no checkable fact under it. The test: strike the clause — if no checkable fact leaves the post, it was gloss; the sentence keeps its fact clauses. Commanded restatements — the core-facts `<ul>`, FAQ answers, who it's for in its commanded form — stand.
- **Metaphoric locative predication** — a posture verb on anything but a physical place, or any verb placing or positioning the record alongside or among other things, or a simile filing it among a class. A physical place can hold a real locative ("Park Center is next to the courts"); records and entries cannot. State the relation literally.
- **Reportative evidentiality** — a document or the record (the posting, the listing, the page, the source, the role, the job, the event) as a sentence's subject under any verb — lists, centers on, stays centered on, belongs with, lines up with, gives, shows, points to, or the record's org or the record itself as sayer of a speech verb ("the club says", "identifies itself as"). A document is never a subject; state the facts bare as settled knowledge.
- **Metadiscursive importance predication** — a clause or heading asserting a fact is important, relevant, or worth attention, or framing one fact as mattering "because" of another, rather than stating each fact bare.
...
1b. Subject-and-verb scan — every sentence, before any other check below. (a) Subject a document (the page, posting, listing, site, address, schedule, source, role, job, event)? Rewrite so the fact stands alone: "A resume is required"; "90 sessions are scheduled"; "X is the focus". (b) Main verb a light stand-in for "is/is at" — sits, gives, holds, places, brings, keeps, offers, carries — where "is", "is at", "costs", or "opens" states the plain fact? Swap to the plain verb: "LA FORME is located at 238 N. Citrus"; "The Kroc Center is in Atlanta". A light verb doing real, literal work stays: "doors open at noon", "the pass costs $40". (c) Subject or object an abstraction — a quality or summary word (scale, format, setting, vibe, mix, value, backdrop) naming no person, org, place, or concrete part of the record, under any verb including "is" or a causative like "gives"? Hand the slot to the thing itself or state the bare fact; the record itself or a concrete part of it (the festival, the gym, the booth) stays — never a word standing in for it (its schedule, program, day).
...
19. Count the internal links. Link every verified internal URL whose matching phrase already appears naturally, per METHODOLOGY item 4. Add no phrase and bend no sentence; leave any other URL unplaced without treating it as a miss or failure.
...
| 2 | Post type main listing | `/<data_filename>` | From the cached `data_filename` on the resolved post type (already in agent memory from site context). Varies per site (`/events`, `/calendar`, etc.). Anchor on a phrase the draft wrote (e.g. "yoga workshops" → `/events`), never the page-type name ("the events calendar" is banned site furniture). |
...
- **Use Pattern 5** when no verifiable category or location fits the sentence. The anchor is still a draft phrase naming the people themselves ("local personal trainers"), never site furniture ("our directory," "browse trainers") and never a description of the page.
...
**Anchor text: a 2-to-5-word phrase already in the written draft — identified, never composed — internal and external alike.** The longer description belongs in `title`, never in the anchor. Never the target's full title, never generic ("here", "this page"), never site furniture or page-type nouns ("member directory", "full jobs board", "the events calendar") — in the anchor or anywhere in its sentence. The anchor is a phrase the draft already contains, as its sentence wrote it — `title` names the destination; the anchor never does. External anchors carry the source's name ("the NSCA guidelines", not "the guidelines").
...
- Invent geo params. Only `lat`+`lng`+`location_value`+`location_type` (sent together — `location_type` required) filter by location. `state_sn`, `state`, `country`, `city`, `region`, `zip` as raw query params are NOT supported — BD ignores them and the URL filters nothing. Anchor text says the city when the URL filters by location. A draft phrase without the city rides a URL filtered another way instead — any shape `Pattern 3 filter params` supports. `location_value` is display-facing (`lat`/`lng` drive the filter) — give it the clean city string ("Los Angeles, CA"), never a street address, never a ZIP.
...
- Bulk-list existing posts to "see what's available" for internal linking — the ban is on new lookups; live post rows already in context from this run's dedup/list calls are fair Pattern 1 targets. Pattern 3 URLs are constructed from ledger category + the post's location values — no lookup needed.
...
A resolved combo also yields each of its dimensions solo as a separate candidate — the more filters resolved, the more distinct targets in hand. Bank the combo and its solos; each rides a different draft phrase. For each distinct matching anchor phrase, build the URL from exactly the dimensions it names — one dimension yields its solo, more than one yields that exact multi-dimension shape; every distinct matching anchor phrase takes its target this way. Most-specific only breaks ties between URLs for the same phrase, never skips a valid target:
...
10. **Image selection — FEATURE image only.** On the `poolImages` path nothing fires here — the call fires in the pre-create batch (Step 12), never its own turn. Steps 1-3 image path (no `poolImages` tool): run METHODOLOGY `Stage 5: Content manufacture (universal)` → `Image strategy` end-to-end. Inline body images: see the `Inline body images` section.
...
12. **Pre-create batch — the message right after source research completes. This turn's only job: ONE message — the linking pass's gate checks, the `poolImages` call when the tool list carries it, and the dupe re-probe, born to fire together; fewer only when the gate message's subjects run out — a member left to a later message is an incomplete batch. No other calls ride this turn.** The gate checks: per METHODOLOGY `Stage 5: Content manufacture (universal)`'s linking pass — only the Pattern 6/4 member-count gates (trusted Pattern 1/2/3/5 links place directly, no check). The `poolImages` call: per `Image strategy` — on the `poolImages` path the image is settled: no `getImageDimensions`, no image dedup. The dupe re-probe: compose the final `post_title` once, to the `BD Blog field reference` title spec; then the `Dedup` section's subject probe fires with the final title's head noun — verdict, consequence, and banking per METHODOLOGY `Stage 2: Duplicate detection` → `Pre-create dupe gate`.
13. **Content manufacture.** Proceed straight from runbook Step 12 — no extra lookups. Follow METHODOLOGY `Stage 5: Content manufacture (universal)`; this file adds blog-specific shape. Output the full post body as visible HTML text in this turn, then run the linking pass over that written body — placing every inventory URL whose phrase the body holds — and emit the finished linked body. Before emitting the finished body, walk every inventory row to the last, emitting exactly one `LINKED` or `UNPLACED` line per row — the linking is not complete before the last row is recorded. `LINKED: <url> — <the exact `<a href="…">…</a>` tag copied verbatim from the emitted body>` only when that tag is actually present in the emitted body; an inventory row with no such tag in the body is `UNPLACED: <url> — <reason>`. This is its own turn, before and separate from the create call.
14. **Create the post** — fires ALONE in its own turn, after Steps 7-13 are complete for the candidate; nothing batches with a create. Set `post_content` to a character-for-character copy of Step 13's emitted body — never regenerated, redrafted, or recomposed here; if that exact copy is not in hand, do not create. Via `createSingleImagePost` per METHODOLOGY `Stage 6: Post creation`, with the field set in the `BD Blog field reference` section. Compose the create against the schema turn 1 fetched — the run's one `getToolSchema` per tool.
...
Retrieval fires in the pool-print message: `listSingleImagePosts property=["post_title","data_id"] property_operator=["contains","eq"] property_value=["<ONE string: every topic's variants joined as a single CSV>","<the resolved blog data_id>"] limit=50 fields_only="post_id,post_title,post_status,post_filename"` — the pool lines' variants merge into element 1 as one comma-joined string (`Personal Trainer,Trainer Cost,Home Workout,Home Gym,Protein Timing,Recovery Meal,...` — 3 × N variants, each 1-3 words); each variant is a substring a matching title would contain — the topic's core noun phrase as titles write it, never an invented compound (`Personal Trainer Match`); the arrays stay two-and-two; N topics = 1 compound call plus N subject probes — a five-topic pool, six calls; every candidate probes here; a subject that shifts at research re-probes; a probe left to a later message is an incomplete first message. The dedup message's score: how many variants ride element 1 plus how many probes fired — 3 × N variants and N probes is full marks; a trimmed variant or skipped probe saves a token and ships a dupe. Plus one subject probe per candidate, batched in the same turn — variant 1 (the head noun) against the title field: `listSingleImagePosts property=["post_title","data_id"] property_operator=["contains","eq"] property_value=["<variant 1>","<the resolved blog data_id>"] limit=50` with the same `fields_only` — a differently-worded dupe still carries its subject in the title; v1 riding the compound never substitutes for the probe. Its verdict line cites it: `no match (variants + subject: Personal Trainer) - survives` — one verdict line per pool line. `total` above the returned row count → re-run once, element 1 carrying each topic's single most distinctive variant — the narrower net surfaces the rows the first cast left unseen.
...
1. **Direct-answer opening.** Two intro `<p>`s, 6-8 sentences split unevenly between them (never an even split): the first `<p>` answers what the headline promises in fresh words — its first ~100 words making clear the article's subject and what the reader can do or decide with it; the second widens with the article's context and concrete consequences. No throat-clearing ("Here's the thing"), no preamble — in either.
...
8. **FAQ block before conclusion.** H2 named in the topic's own words — never the stock "Frequently Asked Questions" — opening with a 2-3 sentence `<p>` stating a fact the questions share, never an announcement ("Here are common questions") — with 3-4 H3 questions, each answering a question the body did not already answer — a new fact, not a rephrased one — and each answered in 40-60 words. High AI-citation density per word.
...
- **Specific member profile** (Pattern 4): `/<user.filename>` — resolve via `searchUsers` only, and only when the agent has a specific known person to deep-link to. Member rows returned by verification calls are never Pattern 4 targets. No bulk-listing members.
