...
After the last create's response, the run is not finished until you emit this — never stop on the create tool call. The receipt fires only when no candidate still owes a create — a due create's message is never the receipt's. A run that created posts but sends no receipt still owes one. Before emitting the receipt, every candidate that reached creation is in exactly one state: created (a `create*` call returned a `post_id` greater than zero) or unmade (dedup hit, unresolvable, or a create that returned no live `post_id`). A created candidate is a `posts` entry; an unmade one is never an entry and lowers `post_create_count`, its reason named in `shortfall_reason`. A candidate that is neither still owes its `create*` call — make it now; never write a `posts` entry for a create you did not call or that returned no live `post_id`.
