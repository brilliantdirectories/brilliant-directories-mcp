...
After the last create's response, the run is not finished until you emit this — never stop on the create tool call. The receipt fires only when no candidate still owes a create — a due create's message is never the receipt's. A run that created posts but sends no receipt still owes one. Each of the goal's slots ends created — a `create*` call returned a `post_id` greater than zero, becoming a `posts` entry — or not, for a stated reason (no candidate found, conflicting input, a failed gate, dedup, or a create that returned no live `post_id`), counted in the shortfall and named in `shortfall_reason`. Never write a `posts` entry for a create you did not call or that returned no live `post_id`.
...
- `posts`: one object per create response that returned a `post_id` greater than zero, with `post_id` and `post_url` copied verbatim from that response — never predicted from the title, never filled from the fields you sent. No such response = no entry (never a placeholder `post_id` of 0). `{"post_id": N, "post_type_id": <data_id>, "post_data_type": <data_type>, "post_type_name": "<post type name>", "post_title": "...", "post_url": "<full live URL from the response>", "post_author_id": N}`. Empty array when none.
