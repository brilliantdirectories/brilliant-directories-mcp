...
12. **Content manufacture.** Proceed straight from runbook Step 11 — no extra lookups. Follow METHODOLOGY `Stage 5: Content manufacture (universal)`; this file adds blog-specific shape (post-format templates, answer-first H2s, FAQ block, internal-link density). **Pre-create batch — the message right after the draft stands: ONE message carries the linking pass's gate checks, the `poolImages` call, and the final-title `listSingleImagePosts eq` check — three tools born to fire together.**
