...
| 5 | Member directory landing — entire directory | `/<getSiteInfo.main_directory_url_relative>` | The directory landing page, cached from the run's `getSiteInfo` call — the `main_directory_url_relative` value with a `/` prefix (e.g. `/search`), never the bare site root `/`; `/` is not a link target, and a second landing-flavored phrase after this URL is placed once stays unlinked. Lists every member, no location or category filter applied. **Takes NO query parameters** — appending `?category[]=...` or `?lat=...` does not work; Pattern 3's filter params apply to POST listings only, never to the member directory. For filtered member directory links, use Pattern 6. |
