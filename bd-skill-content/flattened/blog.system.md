...
After the last create's response, the run is not finished until you emit this — never stop on the create tool call. The receipt fires only when no candidate still owes a create — a due create's message is never the receipt's. A run that created posts but sends no receipt still owes one. Before the receipt, each candidate is either created (a `create*` call returned a `post_id` greater than zero) or explained in `shortfall_reason` (dedup, unresolvable, no live `post_id`) — a candidate that is neither still owes its `create*` call; never write a `posts` entry for a create you did not call.
