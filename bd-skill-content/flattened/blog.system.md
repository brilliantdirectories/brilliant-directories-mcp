...
Cached data feeds Stage 4 category routing and the internal-link inventory — the run's hoarded link candidates: the menu links, and, as each is cached, every URL-PATTERNS Pattern 1/2/3/5 target the run's own data yields (post rows dedup returns, the post type's listing, its category filters, its location filters (from the post's own geocoded lat/lng — for events also its date filter), the directory landing). Pattern 6 holds only its resolved slug ingredients, never an assembled URL, until its gate clears.
