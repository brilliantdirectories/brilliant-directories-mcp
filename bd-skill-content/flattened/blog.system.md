...
Fuzzy-match source category vs the **category ledger** list. ≥70% confidence → carry the LEDGER value forward, never the source's wording. <70% → SKIP the record (do NOT create categories). Bank every category in the ledger into the internal-link inventory as its own Pattern 3 filter candidate — each places only on a draft phrase that matches it by meaning. The resolved category is also banked as a Pattern 6 member-directory ingredient marked "pending gate" (Pattern 6 uses the resolved one only) — the member-count gate in the pre-create batch assembles and verifies it (count >= 1 to link, per URL-PATTERNS).
...
19. Count the internal links (relative `<a href="/...">`). Zero while a body phrase still has an internal target → that phrase went unlinked; link it per METHODOLOGY item 4. Add no phrase and bend no sentence.
...
A resolved combo also yields each of its dimensions solo as a separate candidate — the more filters resolved, the more distinct targets in hand. Bank the combo and its solos; each rides a different draft phrase. For any one phrase, match it to the most specific candidate that phrase supports (a phrase naming two dimensions takes the combo; a phrase naming one takes that solo):
