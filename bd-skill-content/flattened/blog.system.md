...
- `post_create`: `1` (this run's task was creating posts). `post_create_goal`: the requested post count — from the run's instructions, never lowered to match the outcome. `post_create_count`: posts with a `post_id` greater than zero returned by a `create*` response this run — nothing else counts. Every `posts` entry must trace to a `create*` call you actually made; a `post_id` of 0 or missing is not created — drop it, never fabricate an entry for a create you never called.
