...
7. **Geocode.** Nominatim every pooled candidate's address — in the dedup turn's spare slots, else the next turn; fill leftover slots with the retry ladder's next tier as backup. Drop geocodes for dupes. Skip lat/lon on failure.
