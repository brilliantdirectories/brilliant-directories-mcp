...
2. `q="<street>, <city>, <state-name>"` OR `q="<street>, <city>, <country>"`. Catches venues that aren't named in the geocoder but have indexed street addresses.
...
## Normalize the geocoder output before passing to BD

The geocoder returns `country_code` lowercase (`"us"`, `"ca"`, `"gb"`) and state as a full name (`"New York"`, `"Ontario"`). BD's `country_sn` and `state_sn` expect uppercase ISO codes. Normalize directly.

1. **`country_sn`**: uppercase the geocoder's `country_code`. `"us"` → `"US"`, `"ca"` → `"CA"`, `"gb"` → `"GB"`.
2. **`state_sn`**: map the geocoder's state name to its ISO-3166-2 2-letter code (US: `"New York"` → `"NY"`, `"California"` → `"CA"`; Canada: `"Ontario"` → `"ON"`, `"British Columbia"` → `"BC"`; Australia: `"New South Wales"` → `"NSW"`; etc.). Always uppercase. If the country has no state-equivalent (e.g. Malta, Luxembourg, Singapore) or the geocoder returned a sub-region that isn't a standard ISO-3166-2 subdivision, **OMIT `state_sn`** — pass `country_sn` alone.
...
**Section headings:** the event's own facts, program, or specifics — a schedule, season, or recurrence section's heading names the dates, days, or hours themselves; banned dimension labels = Setting, Overview, Details, Background, Logistics, Location, Highlights, About.
...
Events get category, location (`lat`+`lng`+`location_value`+`location_type=locality`), and date (`daterange`) filter dimensions. Date filters are events-only (other post types skip them).
