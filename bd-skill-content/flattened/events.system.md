...
5. **Source discovery.**
    ...
    - **5d. Insurance fill.** Slots still open after the dedup calls (ten per turn) fill with new-angle `WebSearch`es, or `WebFetch`es that pin a viable unkeyed entry's missing keys or open the best list-page(s) — backup candidates for the next cycle in case every tested candidate fails. Nothing else rides this turn.
