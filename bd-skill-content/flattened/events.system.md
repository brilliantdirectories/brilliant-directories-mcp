...
Build the agent's mental model of the site — what it's about, who it serves, its taxonomy, its main navigation — for vertical alignment. The 4 site-context tool calls (`getSiteInfo`, `listTopCategories`, `listPostTypes`, `listMenuItems`) are independent and fully specified here — call them directly in ONE batched round (no `getToolSchema` needed), then process results. Every run ends on a create, so in this same round also fetch its schema up front (`getToolSchema createSingleImagePost`), leaving it in hand for post creation. Numbering is read order, not turn order.
