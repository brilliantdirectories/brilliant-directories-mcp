...
| `post_location` | The display address — full street when known, else city/state (the string that geocoded, e.g. `"Denver, CO"`); lat/lon are the map coordinates. A multi-location source pins the post to ONE location — the location the post's own registration URL's page names; else the source's primary or first-listed — for the title, location, and geocode; the other locations are body facts only. Do NOT prepend the venue name (already in `post_venue`). |
