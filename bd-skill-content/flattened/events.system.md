...
6. **Pool-print turn.** Every WebSearch result showing a title and a start date later than today (inside the user's window; default 90 days) pools as `N. Title — YYYYMMDD — City` — the shown date pools as shown. Print the numbered pool (best-fit, up to 10) and fire in this same message, on `listSingleImagePosts` only: ONE title compound covering all pooled titles, plus one date probe per candidate — the `post_start_date` leg never rides the title call. N pooled = N+1 calls; fewer is an incomplete turn. None pooled → return to Step 5.
