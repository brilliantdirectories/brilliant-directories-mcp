...
8. **Pre-create batch — this turn's only job: call 8a, 8b, and 8c in this ONE message.** One survivor = six calls — its `poolImages` call, its title check, and its four `Geocode ladder` tiers; a survivor with no venue fires the ladder's two-tier branch instead — four calls. Each additional survivor adds its own six (or four) to this same message. No other calls ride this turn.
    ...
    - **8c. Geocode.** Nominatim every survivor with a known street or city — each survivor's GEOCODING.md `Geocode ladder` tiers batched together as backups; the lowest-numbered hit wins per survivor. Skip lat/lon on failure.
