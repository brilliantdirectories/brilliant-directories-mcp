...
| `post_start_date` | Event start datetime `YYYYMMDDHHmmss` (14 digits, event-local wall-clock — see the `Date/time formats` section). Date AND time both live here. The event template renders its date from this field; a candidate that reached creation already cleared METHODOLOGY's Date-sanity gate, so it carries a confirmed future date — pass it. The published start clock time fills the last six digits (7:30 AM start → `073000`; one AM/PM marker on a range applies to both times; PM adds 12 to the hour except 12 PM stays `12`, AM keeps the hour except 12 AM becomes `00` — 3:00 to 8:30 PM → start `150000`); no published clock time → `000000`. BD silently truncates other formats. |
