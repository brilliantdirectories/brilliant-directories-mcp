...
A search round fires its ten queries in one turn; the pool-print turn carries its probes in one message; after the verdicts a turn carries its steps' specified calls and nothing else — the verification, citation, and liveness research those steps command still fires; improvised WebSearch or WebFetch does not. A candidate has later-step calls only after its `no match — survives` verdict; never more pre-verdict calls on — or because of — a candidate whose dedup keys are already known. A step specifying more than ten calls fires them all in its one turn — never split a step's calls. Steps with stated timing (Stage 2 dedup in the pool-print turn) fire at their stated time. Read EVERY result before any new query — qualifying sources routinely rank 5-8. `site:` follows only a domain a result list surfaced (`Image strategy` pexels queries exempt). Negatives strip a known noise class — `-pdf` on probes, one megaboard domain on jobs queries; more trip bot-blocks; a blocked or emptied negated query retries once without them. A snippet already showing the dedup keys is a hit unopened — it pools and probes as-is; opening waits until it survives. Count every other hit only after opening it: live and on-topic; list-pages additionally show current dates and the correct location in the listed entries themselves. A usable candidate is one that clears your type's gates — on-topic, correct location, and any date/recency rule the content-type sets. A round that returns usable candidates has succeeded; even a few, surfaced by the same sources repeating across queries, are your pool; select from them and proceed. Classify only a round that surfaced no usable candidate: error/challenge pages = tooling-blocked → one structurally different retry, then stop labelled "blocked"; clean-but-empty = dry. Ending with less than the target is a successful outcome — report it via `shortfall_reason`. No survivor from a round → the next round uses new angles, never repeats of spent queries.
...
6. **Pool-print turn — this turn's only job.** All WebSearch results showing a title and a start date inside the window — later than today, within the next 90 days unless the user set one — enter ONE pool; the shown date pools as shown, never re-verified first. Worked instance — the result `[Elevate Legacy Festival — Los Angeles](elevatelegacyfestival.com) — MG Studio, 1319 W 11th Street; August 8, 2026` shows a title and an in-window date, so it pools as `3. Elevate Legacy Festival — 20260808 — Los Angeles`; identify every such result the same way: print the numbered pool (best-fit, up to 10) per METHODOLOGY `Candidate pool discipline (universal pattern)`, and this same message fires Step 7's calls for the whole pool — ONE title compound covering every pooled candidate, plus one separate date probe per candidate; the `post_start_date` leg never rides in the title call. The pool-print turn's only legal tool is `listSingleImagePosts` — these two call shapes, nothing else. Once a result shows its title and start date, its research stops until it survives Step 7. Count the keyed results — every one enters the pool, best-fit 10 when more: N pooled = exactly 1 compound + N date probes, all in this message — fewer probes than pooled candidates is an incomplete turn. None to dedup → return to Step 5.
