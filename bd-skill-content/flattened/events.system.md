...
Two candidates, three variants each. A one-candidate pool runs the same call — its three variants alone in element 1.
...
The user invoked the skill with a request like "create event posts on my site" or similar. They may have specified cities, categories, window, or limit. Execute the runbook steps in order. Once a step is resolved, move immediately to the next step. **Only make the tool calls the runbook steps specify — no extras; a later step's specified calls fire in the current turn once their inputs are ready.** On per-event failure, continue to the next event.
...
5. **Source discovery.** Run the `Source candidates` section and METHODOLOGY `Stage 3: Source research` steps 2a-2b, then one batched turn of `WebFetch` on the best list-page(s) — the pool harvests from the opened pages, each entry carrying its title and start date — once title and start date are known, that candidate's source research stops until it survives Step 6. Capture the candidate pool per METHODOLOGY `Candidate pool discipline (universal pattern)` and print the numbered list — Step 6 fires in that same message.
...
7. **Geocode.** Nominatim each survivor once verification confirms its address — in the next turn's spare slots; fill leftover slots with the GEOCODING.md retry ladder's next tiers as backup; the lowest-numbered hit wins. Skip lat/lon on failure.
