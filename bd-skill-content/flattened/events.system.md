...
5. **Source discovery.**
    ...
    - **5d. Insurance fill — same message as 5c.** Slots still open after the dedup calls (ten per turn) fill with new-angle `WebSearch`es — backup candidates for the next cycle in case every tested candidate fails. A `WebFetch` takes a slot only to pin a viable unkeyed entry's missing keys or to open the best list-page. Nothing else rides this turn.
    - **5e. Cycle.** Newly-keyed and newly-found entries pool and dedup on arrival. Repeat until a candidate survives; a sweep-proven-dry market ends the run per `Source candidates`.
