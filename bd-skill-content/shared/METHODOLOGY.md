...
4. **Menu link inventory — one call:** `listMenuItems limit=100` (no filter; follow `next_page` while present). This returns every menu's items in one response, including platform-default menus, with empty-link rows already excluded. Cache `{menu_name → menu_link}` as internal-link candidates; skip rows whose `menu_link` contains `%%%`.
