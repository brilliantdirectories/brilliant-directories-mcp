...
2. **Pexels landscape** — follow **Rule: Image URLs** in the MCP corpus (loaded with every MCP tool) exactly. Short version: `WebSearch query="site:pexels.com/photo <topic>"` (NOT `site:pexels.com/search` — 403 on agent runtimes; and NOT `wide`/`landscape`/`horizontal` keywords in the query — Pexels searches those as photo title/tag terms, not orientation), drill to individual `/photo/<slug>-<id>/` URLs, send the bare canonical `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg` to BD. Orientation cannot be reliably verified from the agent runtime (`<head>` meta and JSON-LD are stripped by WebFetch summarization; download-size dropdown is client-side JS; page `?w=NNNN&h=NNNN` are hero-crop hints, not source dimensions). If a true landscape is critical for the slot and you can't be certain, OMIT `post_image` entirely. Search topic uses per-type keywords (events: `"5k race outdoors"`, `"music festival crowd"`, `"yoga class studio"`, etc.). If the first topic returns sparse or irrelevant results, vary the phrasing — broader/simpler ("5k race" → "group race outdoors"), narrower ("yoga class" → "vinyasa studio mat"), synonyms, adjacent contexts — anything still contextually relevant to the post.
3. **Site-config default** for this post type, if defined.
4. **Omit `post_image`** entirely.

**Orientation rule for feature image slots.** Feature slots (`post_image`, `hero_image`, `cover_photo`, multi-image album photos) require landscape (`w > h`). Inline body slots (`post_content`/`group_desc`, `email_body`) accept `w >= h` (square OK). For **source images** (the original event/article/listing page), use the page's OG `og:image:width`/`og:image:height` meta tags or `srcset` 2x descriptors to verify landscape; reject portrait and fall through. For **Pexels candidates**, orientation cannot be reliably verified from the agent runtime (see **Rule: Image URLs**) — if landscape is critical and you have no confident orientation signal, OMIT `post_image` entirely. Better to ship without an image than land a portrait that breaks the card/hero layout.
