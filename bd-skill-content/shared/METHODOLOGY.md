...
2. **Pexels landscape** — follow **Rule: Image URLs** in the MCP corpus (loaded with every MCP tool) exactly. Canonical workflow: `WebSearch query="site:pexels.com/photo <topic> wide landscape"` (NOT `site:pexels.com/search` — those search-page URLs return HTTP 403 on agent runtimes and are not indexed targets), drill to individual `/photo/<slug>-<id>/` URLs, `WebFetch` each photo page with the rule's exact orientation-verification prompt, then send the bare canonical `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg` to BD. Don't restate the workflow here; trust the corpus rule. Search topic uses per-type keywords (events: `"5k race outdoors"`, `"music festival crowd"`, `"yoga class studio"`, etc.). If the first topic returns sparse or irrelevant results, vary the phrasing — broader/simpler ("5k race" → "group race outdoors"), narrower ("yoga class" → "vinyasa studio mat"), synonyms, adjacent contexts — anything still contextually relevant to the post. Don't hold out for a perfect literal match.
