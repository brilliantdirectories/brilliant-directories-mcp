...
4. **Menu link inventory — one call:** `listMenuItems limit=100` (no filter; follow `next_page` while present). Empty-link rows are already excluded. Keep only rows with a non-null `menu_item_id` — the site's own customized menus; platform-default rows return `menu_item_id: null` and are dropped (server-side filters on this column are silently ignored — filter client-side). Cache `{menu_name → menu_link}` as internal-link candidates; skip rows whose `menu_link` contains `%%%`. Zero rows kept → proceed without menu links.
