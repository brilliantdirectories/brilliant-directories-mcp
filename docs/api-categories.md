...
BD has a 2-level taxonomy mapped to 2 separate API resources (plus an optional join table). Agents get this wrong often — here's the canonical model:
...
1. `createCategory` with `name=Restaurants`, `filename=restaurants` → returns `category_id` (this is what populates a user's `profession_id`).
2. `createService` with `name=Sushi`, `profession_id=<Restaurants category_id>`, `filename=sushi` → returns `service_id`.
3. Assign Alice: either `updateUser` with `profession_id=<Restaurants>` + `services=<Sushi service_id>` (member's primary classification), OR `createUserService` with `user_id=<Alice>` + `service_id=<Sushi>` (adds a join-table row with pricing/specialty).
