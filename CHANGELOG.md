...
Reality check exposed a load-bearing bug class in every location-filtered internal link the content skill has generated to date: `URL-PATTERNS.md` documented `lat`+`lng`+`location_value` as the only required geo params, but BD's URL parser is fragile — without `location_type=locality` (or `location_type=postal_code`), the URL silently breaks and the search-results page returns zero results. Every "more events in {city}" internal link generated by the skill was a dead link.

`location_type` exists primarily as UI plumbing: BD writes it into the sidebar search-form's mode toggle (city vs ZIP button), so the user can see the location auto-filled and refine from there. `lat`/`lng` still do the actual geo filtering. But omitting `location_type` breaks the URL anyway.

Fixes in this release:

- **`URL-PATTERNS.md` table row:** renamed to `lat` / `lng` / `location_value` / `location_type`. Example URL now: `lat=46.7534&lng=-92.0681&location_value=Duluth%2C+MN+55802&location_type=locality`. Cell prose: "Send all four together — `location_type` is required even though `lat`/`lng` do the search ... omit it and BD's URL parser breaks, returning zero results."
- **Default mode** is `location_type=locality` (city-level). `location_type=postal_code` documented as the ZIP-radius alternate for dense-metro cases (NYC, etc.) where city is too broad.
- **State-level filtering is NOT supported** — `state_sn`/`state`/`country` as URL params do nothing; rule explicitly warns the agent off.
- **Anchor-text guidance branched by mode:** if `location_type=locality` say the city; if `location_type=postal_code` say city + ZIP. No "in [State]" / "in [Country]" phrasing.
- **Composition examples updated** to include `location_type=locality` on both geo + combo lines.
- **`events.md` internal-link bullet updated** to match the new four-param shape with the same zero-results consequence callout.

Verification: external minion audited both files and ran 5 quiz scenarios (Austin city link, Times Square ZIP, "in Texas" anti-pattern, category+city combo, forgetfulness drift). 7/7 audit PASS, 5/5 quiz PASS.
...
v6.49.44 said "use a different separator or no separator" — abstract. Added concrete options: `(e.g. comma, hyphen)`. ANTI-SLOP forbids en/em-dashes across body prose; hyphen (U+002D) is safe and is the standard title-separator alternative.
