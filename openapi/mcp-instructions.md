...
- **Orientation — LANDSCAPE required for feature/hero images** (`post_image`, `hero_image`, `cover_photo`, multi-image album photos). Not portrait, not square. Square is acceptable only for `profile_photo` / `logo`. **On Pexels, filter the SEARCH page with `?orientation=landscape` (e.g. `pexels.com/search/mountain/?orientation=landscape`) before picking a photo — that param belongs on the search URL only, NEVER on the final `images.pexels.com/photos/...jpeg` URL you send to BD (imported fields stay bare — see bullet 1).** Do not pick from unfiltered thumbnails — they crop to squares and hide portrait-only originals. If unsure about a URL, skip it.
