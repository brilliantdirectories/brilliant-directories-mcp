  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `Update-tool schemas are DOCUMENTATION, not whitelists — applies to every \`update*\` tool across the entire API. The \`properties\` listed on each update tool's request body name the commonly-edited, enum-tagged, or interaction-annotated fields — they are NOT a server-side allow list. BD's backend accepts any field it recognizes as a column/EAV key on the target resource. If a field appears in the resource's \`get*\` / \`list*\` response but not in the \`update*\` schema, **you can still send it on update and BD will persist it** (verified: e.g. \`updatePostType\` accepts \`feature_categories\` even though the spec doesn't list it). Do not refuse an edit because a field is absent from the schema; do not report "tool-schema limitation" — that is a misread. Workflow when a user asks to change a field not in the update schema: (1) confirm the field exists in the resource's current GET response with a sensible current value; (2) send the update with that field name; (3) confirm round-trip by re-reading the record. Only refuse if the field genuinely doesn't exist on the resource or the user is asking for a structural change the resource doesn't support.\n\nFilters are single-condition only: \`property=<field>&property_value=<val>&property_operator==\`. Multi-condition array-syntax filters are not honored — combine conditions by narrowing server-side with the most selective single field, then CLIENT-SIDE filtering the returned rows by the remaining fields of the pair/triple. Applies equally to users_meta lookups and to every join-table pre-check (createLeadMatch, createTagRelationship, createMemberSubCategoryLink).`,
        ``,
