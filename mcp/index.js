  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `Duplicate silent-accept — pre-check before create (applies to every resource with a natural-key field). **BD does NOT enforce DB-level uniqueness on most natural-key fields.** Live-verified resources that accept identical duplicates silently (the per-tool descriptions spell out each field + the consequence): \`createUser\` (email, when \`allow_duplicate_member_emails\` is ON), \`createTag\` (tag_name within a group_tag_id), \`createUserMeta\` ((database, database_id, key) triple), \`createWebPage\` (filename on list_seo), \`createForm\` (form_name), \`createEmailTemplate\` (email_name), \`createWidget\` (widget_name), \`createMenu\` (menu_name), \`createTopCategory\` (filename), \`createSubCategory\` (filename scoped to profession_id), \`createMembershipPlan\` (subscription_name), \`createTagGroup\` (group_tag_name), \`createSmartList\` (smart_list_name), \`createDataType\` (category_name), \`createRedirect\` (old_filename — plus reverse-rule loop check). Two calls with the same natural key both succeed, produce different primary keys, and leave downstream lookups ambiguous. **Standard pre-check: server-side filter-find, NOT paginate-and-search.** Before every create on these resources: (1) call the corresponding \`list*\` with \`property=<natural-key-field>&property_value=<proposed>&property_operator==\` — this returns one tiny payload regardless of how many records the site has (sites in the wild have thousands of posts/widgets/redirects; dumping full lists wastes rate limit and context), (2) if a match exists: reuse the existing record's ID, OR ask the user, OR pick an alternate and re-check, (3) only if zero rows, proceed with create. Live-observed 2026-04-20: two separate AI sessions with identical user prompts produced two WebPages at the same URL (\`/free-ebook\` twice, different seo_ids, both live) — the filter-find pre-check prevents this deterministically. For \`createUserMeta\` specifically, follow the list→update-if-found / create-if-not 3-step workflow in that tool's description. For \`createRedirect\`, run TWO filter-finds (exact-pair skip + reverse-rule loop prevention) — see tool description.`,
        ``,
