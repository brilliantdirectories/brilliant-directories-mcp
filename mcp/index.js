  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `Form creation recipe — every \`createForm\` call an agent makes MUST follow this recipe, or submissions error out. (1) \`form_url\` = \`/api/widget/json/post/Bootstrap%20Theme%20-%20Function%20-%20Save%20Form\` — exact value, URL-encoded spaces kept as \`%20\`. (2) \`table_index\` = \`ID\`. (3) \`form_action_type\` = \`widget\` (safe default; user may override to \`notification\` or \`redirect\`). Never leave empty for a public-facing form. (4) \`form_email_on\` = \`0\` (agent default OFF; admin UI default is ON but that spams inboxes from AI-generated forms). (5) If \`form_action_type=redirect\`, also set \`form_target\` = destination URL (not schema-enforced, agent MUST remember — BD accepts the create without it and the form silently goes nowhere on submit). (6) When \`form_action_type\` is \`widget\`/\`notification\`/\`redirect\`, the fields list MUST end with \`field_type=ReCaptcha\`, then \`field_type=HoneyPot\`, then \`field_type=Button\` — in that exact order, and these three MUST be the three HIGHEST-ORDERED fields on the form (no other field can have \`field_order\` equal to or greater than theirs). To pick values: call \`listFormFields\`, find the current max \`field_order\`, then use \`max+1\`, \`max+2\`, \`max+3\` for ReCaptcha / HoneyPot / Button. Never add fields AFTER Button — it's always the tail. ReCaptcha and HoneyPot need no configuration beyond \`field_type\` (OMIT \`field_required\`, \`field_placeholder\`, view-flags — BD handles these fields server-side). Without every rule above, BD errors on submit and the form won't function. Audit existing forms before \`updateForm\` flips them into a public-facing \`form_action_type\` — run \`listFormFields\` first to confirm the tail pattern exists.`,
        ``,
