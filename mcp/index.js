...
function aicSlugify(name) {
  ...
  // Built one character at a time: truncating the encoded string instead can cut a
  // multi-byte sequence in half and produce a slug the browser cannot decode.
  let enc = "";
  for (const ch of String(name).trim()) {
    const next = enc + encodeURIComponent(ch).toLowerCase();
    ...
    for (const [method, op] of Object.entries(methods)) {
      ...
      if (method === "get" && op.parameters) {
        ...
        if (hasLimit) {
          ...
          properties.property_value = { anyOf: [{ type: "string" }, { type: "number" }, { type: "boolean" }, { type: "array", items: { anyOf: [{ type: "string" }, { type: "number" }] } }], description: "Value to filter by; array to pair with a `property` array (same length)." };
  ...
  if (seoType !== "data_category") {
    if (
      ...
      if (DATA_CATEGORY_FILENAME_RE.test(oldFilename) && !incomingFilenameSet) {
        return { error: `Transitioning seo_id=${args.seo_id} from seo_type=data_category to seo_type=${seoType} requires a new filename. The current filename '${oldFilename}' is a wrapper-managed placeholder and won't render correctly on the new seo_type. Pass filename=<new-slug> on this update — for seo_type=profile_search_results use a BD-router slug like 'country/state/city/top/sub' (apply **Rule: Member search SEO pages**); for seo_type=content use a human slug like 'about-us'.` };
      }
      return {
        strip_orphans: {
...
function coerceStringifiedFilterArrays(args) {
  ...
  for (const k of FILTER_TRIPLET_KEYS) {
    ...
    if (typeof v === "string" && v.length > 1 && v[0] === "[" && v[v.length - 1] === "]") {
      ...
      // BD's query layer is string-typed; numbers and booleans stringify to match.
      args[k] = String(v);
    }
  }
}
