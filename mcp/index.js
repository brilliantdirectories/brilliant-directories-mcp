  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `**\`revision_timestamp\` — agent must set on every update; BD does NOT auto-populate.** Applies to every resource with a \`revision_timestamp\` field on its GET response: widgets, forms, email templates, top categories, sub-categories, post types, membership plans, users_meta, and similar. On each \`update*\` call, include \`revision_timestamp=<current time>\` in the format \`YYYY-MM-DD HH:mm:ss\` (e.g. \`2026-04-20 19:34:51\`) — this is the widget/form/category shape, DIFFERENT from the \`list_seo.date_updated\` format which uses \`YYYYMMDDHHmmss\` (no separators). Omitting it leaves the timestamp stale, which breaks cache invalidation, "recently updated" sorts, and admin audit trails. \`create*\` calls typically get an initial timestamp server-side, but setting it explicitly on create is always safe and recommended.`,
        ``,
