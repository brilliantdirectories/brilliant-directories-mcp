  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `Pagination (all \`list*\` and \`search*\` endpoints only). Parameter is \`limit\` (records per page) — \`per_page\` is silently ignored and returns the default dump. Param \`page\` = opaque base64 cursor token from the previous response's \`next_page\` field (format: \`base64_encode("{page_num}*_*{limit}")\` — pass back verbatim, never decode or construct). **NEVER pass numeric offsets like \`page=2\`** — the server runs \`base64_decode\` then splits on \`*_*\`; a plain integer decodes to garbage and the server silently resets to page 1, so you'll loop page 1 forever thinking pagination is broken. Verified live. Single-record \`get*\`, create/update/delete don't paginate. **Server-side hard cap on \`limit\` = 100** — default is 25 if omitted, max 100, and values above 100 are SILENTLY CLAMPED (verified: \`limit=150\` on a 118-record table returned exactly 100 records + a \`next_page\` cursor, NOT all 118). Do NOT assume you got everything when \`limit\` > 100 — you didn't; continue via cursor. **When you send \`page\`, \`limit\` is IGNORED** — the page size is baked into the cursor token. To change page size mid-traversal, start over with \`limit=N\` and NO \`page\` param. For context efficiency (Claude tool-result budget): use \`limit=25\` for normal browsing, \`limit=10\` for scan-and-filter loops, \`limit=5\` for sampling; avoid \`limit=100\` unless you genuinely need the max page size — large responses still burn context even within the server cap. Gotchas: (1) \`next_page\` is often non-empty even on the last page — stop when \`current_page >= total_pages\` OR \`message.length === 0\`, NOT when \`next_page === ""\`; (2) \`total\` can drift across consecutive calls on active sites — treat it as approximate; (3) \`total\` and \`total_pages\` are returned by every page and respect the requested \`limit\`. **Efficiency pre-check:** for "how many X are there" questions, call \`list*\` with \`limit=1\` — returns \`total\` in a tiny payload without dumping records. **Sequential-page recipe (copy-paste pattern):** (1) first call: \`listUsers limit=10\` → returns 10 records + \`next_page\` token. (2) next call: \`listUsers page=<that token>\` (no \`limit\`, it's baked in) → records 11-20 + new token. (3) repeat, each response handing you the next cursor. (4) stop when \`current_page >= total_pages\`. Example: 118 members at \`limit=10\` = 12 total pages = 12 calls to enumerate all members with tiny per-call payloads.`,
        ``,
        ...
        ``,
        `**Null / empty-value filters — live-verified no-ops. Don't bother; paginate + client-side filter instead.** Tested on listUsers against a 118-record site: \`property_operator=is_null\` on a NOT-null column (\`first_name\`) returned \`total=118\` (full dataset, filter ignored). \`property_operator=is_not_null\` on the same column returned \`total=118\` (same — filter ignored). \`property_value=""\` with \`operator==\` on a genuinely null column (\`logo\`) returned \`total=118\` (server can't match empty-string-as-null). The \`is_null\`/\`is_not_null\` operators appear in the schema but are silently dropped by the server; agents that use them think they're filtering when they're actually getting the full table back. **Exception — numeric zero-sentinel columns DO filter correctly with \`=\`:** \`property=profession_id&property_value=0\` returned exactly the 8 members with no top category assigned. So when a "missing value" is represented as an integer zero (profession_id=0, subscription_id=0 rare but possible, any numeric FK) use \`=0\` and it works server-side. **For string-null / empty-string / empty-url discovery** (missing \`logo\`, \`profile_photo\`, \`phone_number\`, \`website\`, etc.): paginate with a small \`limit\` (10-25), filter each page client-side, accumulate only \`user_id\` + the evidence field. Never try \`is_null\` or \`property_value=""\` as a shortcut — you'll silently process the entire dataset thinking you filtered.`,
        ``,
        `**Profile-photo detection — \`profile_photo\` and \`logo\` columns are NOT reliable signals; use \`image_main_file\`.** Live-tested: every one of 118 test-site members returned \`profile_photo: null\` and \`logo: null\` at the top level of the user record, even for members with photos visibly rendered on the site. The authoritative field is \`image_main_file\` — BD's resolved URL that falls back to \`https://<site>/images/profile-profile-holder.png\` when no photo exists. Detection logic: member has a real photo IFF \`image_main_file\` is present AND does NOT end with \`profile-profile-holder.png\`. Same placeholder is used site-wide regardless of subscription plan or listing type. Use this pattern for any "find members missing a profile photo" task.`,
        ``,
