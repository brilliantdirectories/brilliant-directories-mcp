  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `Pagination (all \`list*\` and \`search*\` endpoints only). Parameter is \`limit\` (records per page) — \`per_page\` is silently ignored and returns a full/default dump. Param \`page\` = opaque cursor token from the previous response's \`next_page\` field; NEVER numeric offsets like \`page=2\`. Single-record \`get*\`, create/update/delete don't paginate. **BD enforces NO server-side cap on \`limit\`** — passing \`limit=500\` really does return 500 records in one response, and on high-volume resources (users, posts, categories, tags) that routinely exceeds Claude's tool-result token budget and forces early conversation truncation. **Agent MUST self-limit.** Defaults: \`limit=25\` for normal browsing, \`limit=10\` for scan-and-filter loops (find members missing a field, posts with a tag, etc. — smaller pages keep memory low when iterating), \`limit=5\` for quick sampling. Never pass \`limit\` > 50 unless the user explicitly asks to dump everything AND you've verified \`total\` is small first via \`limit=1\`. To process large datasets: loop the cursor with a small \`limit\`, filter client-side per page, accumulate only the fields you need (not entire records). Gotchas: (1) \`next_page\` is often non-empty even on the last page — stop when \`current_page >= total_pages\` OR \`message.length === 0\`, NOT when \`next_page === ""\`; (2) \`total\` can drift across consecutive calls on active sites — treat it as approximate; (3) don't change \`limit\` mid-traversal — \`total_pages\` is computed from the cursor's baked-in page size, and changing \`limit\` on an existing cursor miscounts pages; restart from a fresh cursor when changing page size; (4) \`total\` and \`total_pages\` are returned by every page and respect the requested \`limit\` — use them to size your loop. **Efficiency pre-check:** for "how many X are there" questions, call \`list*\` with \`limit=1\` — returns \`total\` in a tiny payload without dumping records.`,
        ``,
