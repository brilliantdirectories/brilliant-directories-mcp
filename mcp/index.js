...
async function main() {
  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        `Brilliant Directories API — membership/directory sites. Tool descriptions are ground-truth; read the one you're about to call before assuming limits.`,
        ``,
        `ACT, DON'T ASK. For capability questions, check the tool description. For business decisions (who/what/when), ask once and proceed.`,
        ``,
        `NON-OBVIOUS CAPABILITIES — agents miss these:`,
        `• External image URLs are auto-fetched: pass \`auto_image_import=1\` to \`createUser\`/\`updateUser\` with \`profile_photo\`/\`logo\`/\`cover_photo\` URLs. Do not download client-side.`,
        `• Categories auto-create by NAME on \`createUser\`: pass \`profession_name="X"\` and \`services="Sub1,Sub2"\` — no pre-call to \`createTopCategory\`/\`createSubCategory\` needed. On \`updateUser\`, add \`create_new_categories=1\` to enable the same behavior.`,
        `• Sub-sub-category syntax: \`services="Parent=>Child,Other"\`. Full 3-tier in one call.`,
        `• Welcome emails: off by default on API creates. Pass \`send_email_notifications=1\` when needed.`,
        `• Profile URL = \`<site>/<user.filename>\`. Filename is the full relative path. Do NOT prepend \`/business/\`, \`/profile/\`, etc.`,
        ``,
        `HIERARCHY NAMES (BD internals ≠ tool names):`,
        `• Top-level category → \`TopCategory\` tools (BD: profession/\`list_professions\`). No \`createProfession\` tool exists.`,
        `• Sub-category → \`SubCategory\` tools (BD: service/\`list_services\`). Requires \`profession_id\`. No \`createService\` tool.`,
        `• Member↔sub link with metadata → \`MemberSubCategoryLink\` (BD: \`rel_services\`). Without metadata, just set \`services\` CSV on the user.`,
        ``,
        `POST ENDPOINT ROUTING — call \`listPostTypes\`/\`getPostType\` first, check \`data_type\`:`,
        ...
        `• Others (10/13/21/29) → admin-internal, not post-creatable`,
        ``,
        `RATE LIMIT: 100 req/60s (raisable to 1000/min via BD support, not self-service). On 429, back off 60s+. For bulk ops, pace yourself; call \`verifyToken\` first on large jobs.`,
        ``,
        `PAGINATION: \`limit\` (default 25, max 100) + \`page\` (opaque cursor from previous response's \`next_page\`). Never numeric offsets.`,
        ``,
        `WRITES ARE LIVE. Confirm before bulk delete/update. Prefer \`updateUser active=3\` (Canceled, reversible) over \`deleteUser\`.`,
      ].join("\n"),
    }
  );
