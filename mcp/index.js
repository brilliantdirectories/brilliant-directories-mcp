  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `Member Listings post type (\`data_type=10\`, singleton per BD site) — the only post type with NO profile/detail page of its own. Controls the Member Search Results page UI/UX only (members render via BD's core member profile system, not via a post-type template). When a user says "change X on my member search results pages," the edit path is: \`listPostTypes\` filtered \`property=data_type&property_value=10&property_operator==\` → receive the single record with its site-specific \`data_id\` → \`updatePostType\` with that \`data_id\` + the changed field(s). Cache the \`data_id\` for the session (never changes for a site). **Commonly-edited Member Listings fields:** 12 settings (\`h1\`, \`h2\`, \`per_page\`, \`keyword_search_filter\`, \`enableLazyLoad\`, \`category_order_by\`, \`category_ignore_search_priority\`, \`post_type_cache_system\`, \`category_sidebar\`, \`sidebar_search_module\`, \`sidebar_position_mobile\`, \`enable_search_results_map\`) + 3 search-results code fields (\`category_header\`, \`search_results_div\`, \`category_footer\`). Any other column returned by \`getPostType\`/\`listPostTypes\` is also writable (per the universal update-schema-is-documentation rule) — send whatever the user asks to change. Omit \`profile_header\`/\`profile_results_layout\`/\`profile_footer\` / \`search_results_layout\` on Member Listings — they're harmless if sent but have no rendering effect here (Member Listings has no profile/detail page of its own). Do NOT mutate structural fields (\`data_type\`, \`system_name\`, \`data_name\`, \`data_active\`, \`data_filename\`, \`form_name\`, \`icon\`, \`software_version\`, \`display_order\`).`,
        ``,
