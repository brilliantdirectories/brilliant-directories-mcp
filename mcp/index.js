...
async function main() {
  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        `Brilliant Directories API — manage BD-powered membership/directory sites (members, posts, leads, reviews, categories, pages, emails, forms, widgets, menus, tags, redirects, and more).`,
        ``,
        `## Before claiming a limitation — READ THE TOOL DESCRIPTION FIRST`,
        `Do NOT say "I can't do X" or "the API doesn't support Y" based on typical REST API assumptions. This server documents BD-specific capabilities that are NOT obvious from tool names. Notable examples:`,
        `- \`createUser\` / \`updateUser\` accept external image URLs for profile_photo/logo/cover_photo and auto-fetch them locally when you pass \`auto_image_import=1\`. You do NOT need to download images client-side.`,
        `- Categories and sub-categories auto-create by NAME on \`createUser\` (hardcoded). You do NOT need to call \`createTopCategory\`/\`createSubCategory\` first if you're creating a member into a new category — just pass \`profession_name\` and \`services\` strings.`,
        `- \`services\` on create/update supports sub-sub-category nesting via \`parent=>child\` syntax (e.g. \`services="Honda=>2022,Toyota"\`).`,
        `- \`send_email_notifications=1\` on \`createUser\` triggers welcome emails (off by default so API creates are silent).`,
        `- Profile URLs are built from just \`<site-domain>/<user.filename>\` — the filename field already contains the complete relative path; do NOT invent \`/business/\` or \`/profile/\` prefixes.`,
        `Scan the relevant tool description end-to-end before responding to capability questions — 9 times out of 10 the capability exists.`,
        ``,
        `## Rate limits`,
        `100 requests per 60 seconds per API key (default). Customers can request a higher limit from BD support (any value 100–1,000/min) — NOT a self-service setting in admin.`,
        `On HTTP 429, back off at least 60 seconds before retrying. For bulk operations, pace requests below the configured limit.`,
        `Before large jobs, call \`verifyToken\` first to confirm credentials and headroom.`,
        ``,
        `## Pagination`,
        `Cursor-based via \`limit\` + \`page\` tokens. Default limit 25, max 100. Do NOT assume numeric page offsets — the \`page\` parameter is an opaque token from \`next_page\` in the previous response.`,
        ``,
        `## Write safety`,
        `Destructive operations (delete, bulk update, mass import) apply to LIVE production data. Confirm with the user before running bulk deletes, mass updates, or any operation affecting >50 records. Prefer \`updateUser\` with \`active=3\` (Canceled status) over \`deleteUser\` when reversibility matters.`,
        ``,
        `## Category hierarchy (3-tier, agents get this wrong often)`,
        `Top Category (profession, \`list_professions\`) → \`createTopCategory\`/\`listTopCategories\`.`,
        `Sub Category (service, \`list_services\`) → \`createSubCategory\`/\`listSubCategories\`. Requires \`profession_id\`.`,
        `Member↔Sub link (rel_services) → \`createMemberSubCategoryLink\` when per-link metadata (price, specialty, completion count) is needed; otherwise just set the member's \`services\` CSV via \`updateUser\`.`,
        `There is NO \`createProfession\` or \`createService\` tool — those are BD internal DB names. Use the explicit \`TopCategory\`/\`SubCategory\` names documented above.`,
        ``,
        `## Post type families (pick the right create endpoint)`,
        `Every post type in \`data_categories\` has a \`data_type\` field (integer) that determines its family:`,
        `- \`data_type=4\` → Multi-image family (albums, galleries, Property, Product) → \`createMultiImagePost\``,
        `- \`data_type=9\` or \`20\` → Single-image family (blog, event, job, coupon, video) → \`createSingleImagePost\``,
        `- Other values (10, 13, 21, 29) → internal admin types, not creatable as posts via this MCP`,
        `Call \`listPostTypes\` or \`getPostType\` first to see the target post type's \`data_type\`, then pick the correct create endpoint.`,
      ].join("\n"),
    }
  );
