  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `**Member Listings post type (\`data_type=10\`, singleton per BD site)** — the only post type with NO profile/detail page of its own. Controls the Member Search Results page UI/UX only (members render via BD's core member profile system, not via a post-type template). Edit path: \`listPostTypes property=data_type property_value=10 property_operator==\` → receive the single record with its site-specific \`data_id\` → \`updatePostType\` with that \`data_id\` + the changed field(s). Cache \`data_id\` for the session (never changes for a site).\n\n**Member Listings cheat-sheet (reference, NOT a limit — every column returned on GET is writable per the universal schema-is-documentation rule):** BD admins most commonly tune 12 UI/UX settings (\`h1\`, \`h2\`, \`per_page\`, \`keyword_search_filter\`, \`enableLazyLoad\`, \`category_order_by\`, \`category_ignore_search_priority\`, \`post_type_cache_system\`, \`category_sidebar\`, \`sidebar_search_module\`, \`sidebar_position_mobile\`, \`enable_search_results_map\`) + 3 search-results code fields (\`category_header\`, \`search_results_div\`, \`category_footer\`). If a user asks about a field not in this cheat-sheet (\`feature_categories\`, \`icon\`, \`category_tab\`, etc.) — just send it; BD persists any real column. The cheat-sheet saves a discovery GET when the user asks "what can I tweak"; it does NOT restrict what you can write.\n\n**Member Listings-specific guardrails (apply ONLY to \`data_type=10\`, NOT to any other post type):** \`profile_header\` / \`profile_results_layout\` / \`profile_footer\` / \`search_results_layout\` have no rendering effect on Member Listings (no profile/detail page) — harmless if sent, but useless; skip them. On every OTHER post type (blog, event, coupon, property, product, etc.) these ARE legitimate, rendering fields — write them freely there.\n\n**Universal post-type safety (applies to EVERY post type on every \`updatePostType\` call, not just Member Listings) — do NOT mutate these structural fields on any post type:** \`data_type\`, \`system_name\`, \`data_name\`, \`data_active\`, \`data_filename\`, \`form_name\`, \`software_version\`, \`display_order\`. BD system-seeds these on post-type creation; changing any of them breaks rendering across the site.`,
        ``,
