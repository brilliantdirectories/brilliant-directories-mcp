  ...
  const server = new Server(
    ...
    },
    {
      ...
      instructions: [
        ...
        ``,
        `**Timestamps — agent must set on every update; BD does NOT auto-populate. Two field names, two formats, resource-dependent — always match the format you observe on GET.** Live-verified 2026-04-20 across 8 resources: (1) **\`revision_timestamp\`** — format \`YYYY-MM-DD HH:mm:ss\` (dashes + colons, e.g. \`2026-04-20 19:34:51\`). Universal across widgets, forms, email templates, top categories, sub-categories, post types, membership plans, users_meta, AND list_seo WebPages — every resource that carries this field uses this format. (2) **\`date_updated\`** — format is RESOURCE-DEPENDENT: on widgets it's \`YYYY-MM-DD HH:mm:ss\` (same as revision_timestamp); on list_seo WebPages it's \`YYYYMMDDHHmmss\` (NO separators, e.g. \`20260420193451\`). Same field name, different format. Always verify by reading the GET response first. (3) **\`date_added\`** on users_meta is \`YYYYMMDDHHmmss\` (no separators) — set on create if modifying history. **Which resource has which field:** widgets carry BOTH \`revision_timestamp\` AND \`date_updated\` (both dashes-and-colons); WebPages carry BOTH \`revision_timestamp\` (dashes-and-colons) AND \`date_updated\` (no-separators); forms/email templates/categories/subcategories/post types/membership plans/users_meta carry only \`revision_timestamp\` (dashes-and-colons). On every \`update*\` call, set the current time in whichever fields that resource exposes, using the format observed on GET. Omitting these breaks cache invalidation, "recently updated" sorts, and admin audit trails. \`create*\` calls typically get initial timestamps server-side; setting explicitly on create is safe.`,
        ``,
