...
**Identity-confirming fields on real-person/real-business records — verified source or OMIT.** When creating or updating a member (`createUser` / `updateUser`) or any record that represents a specific real person or real business, the fields `profile_photo`, `logo`, `website`, `facebook`, `twitter`, `instagram`, `linkedin`, `youtube` are IDENTITY-CONFIRMING. Stock photos and fabricated URLs are never valid — a Pexels photo on a real member's `profile_photo` or an invented domain on `website` misrepresents the person. The rule is binary: a value is valid only if it came from THIS person's actual web presence (their site, About page, social profile). If you can't verify, OMIT the field and tell the user "no confirmed [photo/website/social] found for [name] — record created without it; add later when verified." This rule overrides the image-sourcing priority order above for these specific fields. Do NOT fall back to Pexels stock for `profile_photo` on a real person, and do NOT invent domain names for `website`.
