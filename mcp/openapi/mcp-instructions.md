...
**Plan signup/checkout URL.** Compose `getSiteInfo`'s `full_url` + `/` + `getSiteInfo`'s `default_checkout_url` + `/` + the plan's `custom_checkout_url` (from `getMembershipPlan`) if set, else its `subscription_id` — e.g. `https://site.com/checkout/2`. `default_checkout_url` varies by site (`checkout`, `join`, `signup`, …) — use the returned value, never a hardcoded guess.
