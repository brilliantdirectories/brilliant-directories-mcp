...
- **Orientation gate for Pexels-sourced feature image slots.** `post_image`, `hero_image`, and multi-image album photos require landscape — verify via `getImageDimensions` per **Rule: Image dimensions**. `cover_photo` (User banner) is identity-context — sourced from the subject's own web presence per **Rule: Identity-confirming fields**, landscape preferred but not strictly gated, WebP accepted. Inline `<img>` in Froala body (`post_content`, `group_desc`) and email-template body (`email_body`) have no orientation gate — landscape or square preferred, portrait acceptable. `profile_photo` / `logo` are identity-confirming (see **Rule: Identity-confirming fields**); Pexels stock is forbidden for those fields.
- **Pexels sourcing workflow.** Use this when an image is needed and **Rule: Image sourcing** routes to Pexels. (1) `WebSearch query="site:pexels.com/photo <1-3 word topic>"` — Pexels does NOT respect keyword orientation filters in either the query or its on-site search (no `wide`/`landscape`/`horizontal` keyword reliably surfaces landscape-only; those words search the photo's title/tags, not orientation). Returns ~10 result URLs of the shape `https://www.pexels.com/photo/<slug>-<id>/`. (2) For each result URL, extract `<id>` (trailing digits before the final `/`) — this is the candidate photo ID. (3) Construct the bare canonical `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg`. (4) Call `getImageDimensions` per **Rule: Image dimensions** — pass only on `status: "success"` + `orientation === "landscape"`; any other outcome drops the candidate. (5) **Rotate.** Never reuse an ID from earlier in the same skill run. Vary the search topic across posts (`pilates class` → `pilates reformer` → `pilates studio`). If the WebSearch pool runs short, run a new WebSearch with a varied topic phrase. (6) Survivors flow to **Rule: Image dedup** before BD commit. (7) If WebSearch returns nothing and a second varied query also returns nothing, say "no Pexels result for [topic]" and skip — do not guess at random photo IDs. **User-supplied URLs (Pexels or other):** use as-is — user choice is the authority. **Batch submissions** (multi-image CSV): run the workflow for each slot; if a slot returns nothing usable, drop it and name it. Refuse the whole batch only if zero slots fill. **Why this routing:** `www.pexels.com/search/*` returns HTTP 403 to agent-side WebFetch on common runtimes (claude.ai); `WebSearch site:pexels.com/photo` reaches Pexels via Google's index and is not gated.
...
**Two-step usage for Pexels-sourced feature image slots** (`post_image`, `hero_image`, multi-image album photos via `createMultiImagePost.post_image` CSV + `createMultiImagePostPhoto.original_image_url`). NOT for `cover_photo` / `profile_photo` / `logo` — those are identity-context per **Rule: Identity-confirming fields** (sourced from the subject's own web presence, no orientation gate, WebP accepted):
