...
**Aim for Outlook-safe email HTML.** Outlook is the most popular email client AND the most restrictive — if a template displays correctly in Outlook, Apple Mail / Gmail / etc. render it fine too. Design every template to that constraint; the rules below all flow from it.

**`email_body` is content-only — BD provides the document scaffold.** BD renders every template inside a parent `<td>` cell with the full scaffold (doctype + html/head/body) already in place. Open `email_body` with whatever content tag fits — `<p>`, `<table>`, `<div>`, `<h1>`/`<h2>`/etc., `<img>`, mixed structure, anything you'd put inside a rendered email. Skip the scaffold itself; including it double-wraps and breaks rendering. Images follow **Rule: Image URLs** the same way they do in `post_content` — Pexels stock is allowed for decorative imagery.

**Inline `style=""` only — no `<style>` blocks, no `class=""`.** Outlook strips `<style>` and ignores most rules outside the inline `style` attribute; emails have no site-wide stylesheet, so class names match nothing. Every styling declaration lives on the element via the `style` attribute.
...
**No preheader / hidden-element tricks — BD strips them.** Hidden elements (`display:none`, `max-height:0; overflow:hidden`, font-size:1px collapse tricks) are stripped server-side, so the standard inbox-preview-text pattern doesn't survive in BD emails. Don't include a hidden preheader `<div>`; whatever you'd put there is wasted bytes.
