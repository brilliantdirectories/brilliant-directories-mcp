...
**Aim for Outlook-safe email HTML.** Outlook is the most popular email client AND the most restrictive — if a template displays correctly in Outlook, Apple Mail / Gmail / etc. render it fine too. Design every template to that constraint.

**`email_body` is content-only — BD provides the document scaffold.** BD renders every template inside a parent `<td>` cell with the full scaffold (doctype + html/head/body) already in place. Open `email_body` with content directly (`<p>`, `<div>`, `<h1>`/`<h2>`/etc., `<img>`, or — most often — the section tables described below). Skip the scaffold itself; including it double-wraps and breaks rendering. Images follow **Rule: Image URLs** the same way they do in `post_content` — Pexels stock is allowed for decorative imagery.

**Each section of the email is its own top-level `<table>` in `email_body` — no parent element of any kind wraps these tables.** The section tables sit as siblings directly inside what BD provides as the body container; any agent-added wrapper (table, div, span, anything) collapses the sibling pattern and breaks Froala's drag-reorder behavior. Output shape: `<table width="100%" style="width:100%;">...</table>` repeated as siblings, one per logical section (hero, intro, member card, CTA, footer). Both the HTML `width` attribute AND inline `style` width are required — older Outlook reads the HTML attribute, modern clients use the CSS. BD's template (`notemplate ∈ {0, 2, 3, 4}` — **default to `2`** unless the user specifies otherwise or asks for plaintext) already wraps `email_body` in a 600px-wide containing table; your sections fill that width automatically — never add your own max-width wrapper. **User phrases that map to `notemplate=2`:** "template center," "BD template," "use the site email template," "branded email," or any explicit "`notemplate=2`." On any of these, create with `notemplate=2` and write only inner content sections — do NOT add a full email wrapper, background table, or 600px outer container. `notemplate=1` (plaintext-only) ships HTML unwrapped — no template, no logo, no 600px wrapper — so this richly-styled section pattern doesn't apply there; emit minimal HTML or plain `<p>` chains instead. **`notemplate` is immutable on `updateEmailTemplate`** — BD silently ignores attempts to change it after create. To switch wrapper modes, create a replacement template with the correct `notemplate` value and migrate the body content; do not promise the user that an `updateEmailTemplate notemplate=...` call will work.

**Inline `style=""` only — no `<style>` blocks, no `class=""`.** Outlook strips `<style>` and ignores most rules outside the inline `style` attribute; emails have no site-wide stylesheet, so class names match nothing. **`border-radius` IS allowed** — Outlook (Word renderer) ignores it and shows square corners; modern clients render the rounded corners. Use it freely on buttons, cards, avatars, etc.

**Backgrounds beyond solid colors need a solid `background-color:` fallback.** Outlook ignores `background: linear-gradient(...)`, `background-image: url(...)`, and other non-solid `background:` shorthand values. On any section, card, hero, or other element where you want a gradient or image background, always pair it with a solid `background-color:` declared FIRST so Outlook falls back cleanly. Example: `style="background-color:#0A2540; background:linear-gradient(135deg,#0A2540,#1E5BC6);"`. Outlook reads only the first declaration; modern email clients use the gradient or image.

**Do NOT include a hidden preheader `<div>` — BD strips `display:none` server-side.** The standard inbox-preview-text pattern doesn't survive; whatever you'd put there is wasted bytes.
