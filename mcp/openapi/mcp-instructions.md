...
**`email_body` is content-only — BD provides the document scaffold.** BD renders every template inside a parent `<td>` cell with the full scaffold (doctype + html/head/body) already in place. Open `email_body` with whatever content tag fits — `<p>`, `<table>`, `<div>`, `<h1>`/`<h2>`/etc., `<img>`, mixed structure. Skip the scaffold itself; including it double-wraps and breaks rendering. Images follow **Rule: Image URLs** the same way they do in `post_content` — Pexels stock is allowed for decorative imagery.

**Each email section is its own top-level `<table width="100%" style="width:100%;">` — do NOT wrap the whole email in one outer table.** Froala (BD's email editor) treats top-level `<table>` siblings as draggable section units. An email with one big outer table is one immovable block; the user can't reorder hero / spotlight / footer / etc. independently. Output each logical section (hero, intro, member card, CTA, footer) as a separate sibling `<table width="100%" style="width:100%;">` at the top of `email_body` — both the HTML `width` attribute and the inline `style` width are needed (older Outlook reads the HTML attribute, modern clients use the CSS). BD's template (`notemplate ∈ {0, 2, 3, 4}`) already wraps the body in a 600px-wide containing table — your sections fill that width automatically; don't add your own max-width wrapper or you'll double-wrap. `notemplate=1` (plaintext-only) ships HTML unwrapped — no template, no logo, no 600px wrapper — so this richly-styled section pattern doesn't apply there.
...
**No hidden preheader `<div>` — BD strips `display:none` server-side.** The standard inbox-preview-text pattern doesn't survive; whatever you'd put there is wasted bytes.
