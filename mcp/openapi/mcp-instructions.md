...
- **Orientation preference depends on slot.** Feature image slots (`post_image`, `hero_image`, `cover_photo`, multi-image album photos via `createMultiImagePost` CSV + `createMultiImagePostPhoto.original_image_url`) PREFER landscape (`w > h`) — landscape is the design intent for cards/heroes/album grids. When orientation can be verified (source-page OG meta tags, `srcset` 2x descriptors), prefer landscape candidates; when it can't be verified (most Pexels candidates per the sourcing workflow below), take what you find. Inline `<img>` in Froala body (`post_content`, `group_desc`) and email-template body (`email_body`) prefer landscape OR square (`w >= h`); portrait still works but is least preferred. `profile_photo`/`logo` are identity-confirming (see **Rule: Identity-confirming fields**) — orientation preference does not apply (square is typical), Pexels stock is forbidden for these fields entirely.
- **Pexels sourcing workflow.** Use this when an image is needed and **Rule: Image sourcing** routes to Pexels. (1) `WebSearch query="site:pexels.com/photo <1-3 word topic>"` — Pexels does NOT respect keyword orientation filters in either the query or its on-site search (no `wide`/`landscape`/`horizontal` keyword reliably surfaces landscape-only; those words search the photo's title/tags, not orientation). Returns ~10 result URLs of the shape `https://www.pexels.com/photo/<slug>-<id>/`. (2) For each result URL, extract `<id>` (trailing digits before the final `/`) — this is the candidate photo ID. (3) **Orientation cannot be reliably verified from the agent runtime.** `<head>` meta (`og:image:width`/`og:image:height`) is stripped by WebFetch's markdown converter. JSON-LD `<script>` blocks are also stripped. The "Free Download" size dropdown is client-side JavaScript and not in the static HTML. Page-level `?w=NNNN&h=NNNN` query params in body srcset URLs are HERO-CROP HINTS describing how the page renders the photo, NOT the source image's native dimensions — a portrait selfie and a landscape photo both display in the same `1260×750` hero crop. Do NOT use those as orientation evidence. (4) Pick a candidate. (5) **Rotate.** Never reuse an ID from earlier in the same skill run. Vary the search topic across posts (`pilates class` → `pilates reformer` → `pilates studio`). If the WebSearch pool runs short, run a new WebSearch with a varied topic phrase. (6) Send the bare canonical `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg` to BD (wrapper auto-strips `?query` on imported fields; write bare anyway so corpus and wire match). **Format fallback:** if the `.jpeg` URL HEAD-checks as 404, try `.png` then `.webp` — Pexels stores ~10-15% of photos as PNG or WebP originals and only the matching extension serves. If none of the three return 200, reject this ID and try the next candidate. (7) **Take the candidate even if orientation can't be verified.** Landscape is the design preference; image-on-post is the goal. (8) If WebSearch returns nothing and a second varied query also returns nothing, say "no Pexels result for [topic]" and skip — do not guess at random photo IDs. **User-supplied URLs (Pexels or other):** use as-is — user choice is the authority. **Batch submissions** (multi-image CSV): run the workflow for each slot; if a slot returns nothing usable, drop it and name it. Refuse the whole batch only if zero slots fill. **Why this routing:** `www.pexels.com/search/*` returns HTTP 403 to agent-side WebFetch on common runtimes (claude.ai); `WebSearch site:pexels.com/photo` reaches Pexels via Google's index and is not gated.
