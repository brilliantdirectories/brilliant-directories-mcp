...
A `seo_type=data_category` page attaches custom SEO content to a post type's main search-results page or to one of its category-specific pages. Pin via `linked_post_type` (post type's `data_id` from `listPostTypes`) plus `linked_post_category` (either the literal `post_main_page` for the main page, or an exact case-sensitive category name from the post type's `feature_categories`). Wrapper auto-defaults `linked_post_category=post_main_page` when omitted on a fresh create or content→data_category switch. Wrapper enforces pair-uniqueness on `(linked_post_type, linked_post_category)` — only ONE data_category page per combo; transitioning a page AWAY from `data_category` releases the slot automatically. `filename` is OPTIONAL on this seo_type — the public URL is the post type's `data_filename` + (for category pinning) `?category[]=<Exact Category Name>` query string; BD auto-generates a placeholder `list_seo.filename` and a 301 redirect from the placeholder to the canonical URL. To link from page bodies to a post type's category page, write the URL as `<data_filename>?category[]=<Exact Category Name>` (e.g. `articles?category[]=Category 1`). Same hero / SEO meta defaults as `profile_search_results` pages apply.
