...
**Every Pexels URL in `email_body` MUST end with `?auto=compress&cs=tinysrgb&w=1200`** — every placement, every extension (`.jpeg` and `.png` both accept the query string and serve compressed). Forbidden shapes regardless of 200 status: any URL with `dl=`, `fm=`, or attribution params; bare canonical URLs without the query string. These are master files (5-10MB, clip Gmail past 102KB). If the optimized URL fails, swap the photo — never downgrade the URL shape. Same for other stock CDNs (Unsplash, etc.) — always the provider's optimized form.
