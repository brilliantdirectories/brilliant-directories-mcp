...
- **Pexels sourcing workflow.** Use this when an image is needed and **Rule: Image sourcing** routes to Pexels. (1) `WebSearch query="site:pexels.com/photo <1-3 word topic> wide landscape"` (use words like `wide`, `landscape`, `group`, `crowd` in the query — Pexels titles landscape photos with these terms and Google indexes them). Returns ~10 result URLs of the shape `https://www.pexels.com/photo/<slug>-<id>/`. (2) For each result URL, extract `<id>` (trailing digits before the final `/`) — this is the candidate photo ID. (3) **Verify orientation** by `WebFetch <photo-page-URL>` and prompt the fetch for "every URL in the page content whose path is `/photos/<id>/` (use the candidate id) AND whose query string contains both `w=` and `h=` — return each verbatim, one per line." Parse `w=NNNN` and `h=NNNN` from the returned URL. Apply the slot's orientation rule above: feature slot → accept iff `w > h`; body/email slot → accept iff `w >= h`. If WebFetch returns no matching URL, or orientation fails, reject and move to the next candidate. (4) **Rotate.** Never reuse an ID from earlier in the same skill run. Vary the search topic across posts (`pilates class` → `pilates reformer` → `pilates studio`). If the WebSearch pool runs short, run a new WebSearch with a varied topic phrase. (5) Once accepted, send the bare canonical `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg` to BD (wrapper auto-strips `?query` on imported fields; write bare anyway so corpus and wire match). **Format fallback:** if the `.jpeg` URL HEAD-checks as 404, try `.png` then `.webp` — Pexels stores ~10-15% of photos as PNG or WebP originals and only the matching extension serves. If none of the three return 200, reject this ID and try the next candidate. (6) If WebSearch returns nothing and a second varied query also returns nothing, say "no confirmed landscape image found for [topic]" and skip — do not guess. **User-supplied URLs (Pexels or other):** use as-is — user choice is the authority. **Batch submissions** (multi-image CSV): run the workflow for each slot; if a slot returns nothing usable, drop it and name it. Refuse the whole batch only if zero slots fill. **Why this routing:** `www.pexels.com/search/*` returns HTTP 403 to agent-side WebFetch on common runtimes (claude.ai); `WebSearch site:pexels.com/photo` reaches Pexels via Google's index and is not gated. The body's `images.pexels.com` URL with `w=`/`h=` survives WebFetch's markdown extraction; the `<head>`'s `og:image` does not.
