...
- **Orientation — LANDSCAPE only, never portrait/vertical** for every content image: `post_image`, `hero_image`, `cover_photo`, multi-image album photos (`createMultiImagePost` CSV + `createMultiImagePostPhoto.original_image_url`), AND inline `<img>` in Froala body fields (`post_content`, `group_desc`). Portrait breaks article/card/hero/album-grid/body-flow layouts. `profile_photo` / `logo` are identity-confirming headshots/icons (see image-sourcing ladder tier 2-3 rules) — orientation rule doesn't apply, and Pexels stock is forbidden for these fields entirely. **Deterministic landscape verification on Pexels:** (1) Fetch search results with `?orientation=landscape` filter, e.g. `https://www.pexels.com/search/mountain/?orientation=landscape`. (2) Before committing, fetch the photo's individual page (`https://www.pexels.com/photo/<slug>-<id>/`) and read the **`<meta property="og:image:width">` and `<meta property="og:image:height">`** tags in the page `<head>` — these expose the MASTER asset dimensions, not a preview crop. Require `og:image:width / og:image:height >= 1.3`. Do NOT trust the on-page preview URL's `?w=NNNN&h=NNNN` — Pexels normalizes filtered previews to a fixed crop (typically 1260×750) regardless of master orientation, so the preview URL tells you nothing about the source asset. Search-thumbnails crop to squares and can hide portrait masters; never pick from them. The `?orientation=landscape` filter belongs on the SEARCH URL only; the final `images.pexels.com/photos/...jpeg` URL you send to BD must be bare (see bullet 1 — wrapper auto-strips query strings on imported fields). Always strip URL to canonical form `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg`. **User-supplied Pexels URLs:** extract the `<id>` from the path, drill the photo page, run the same `og:image` ratio check. **User-supplied non-Pexels URLs:** image-sourcing ladder tier 1 wins (use it), but include `_orientation_warning: "landscape not verified — non-Pexels source"` in your response so the user can spot-check. **Batch submissions** (multi-image CSV): verify each URL independently; drop unverifiable URLs and submit the verified subset, naming the dropped ones. If zero verify, refuse the whole batch. **Photo page unreachable** (404, network): treat as unverifiable — do NOT fall back to thumbnail or preview-URL dimensions. If you can't confirm `og:image:width / og:image:height >= 1.3`, do NOT pick — say "no confirmed landscape image found for [topic]" and let the user decide.
...
1. Search with `?orientation=landscape` filter — e.g. `https://www.pexels.com/search/<term>/?orientation=landscape`.
2. Open a candidate's individual page and read `?w=NNNN&h=NNNN` from the preview-image URL — confirm `w > h`. Search thumbnails crop square; don't pick from them.
3. Pick the topical, safe-for-work, unwatermarked result.
4. Send the bare `https://images.pexels.com/photos/<id>/pexels-photo-<id>.jpeg` URL for imported fields; inline body uses `?w=700` retina variant.
