...
**User image fields are plan-gated — pick the supported one or skip.** When importing an image for a user (no explicit field named by the user), the user's membership plan governs which image fields are even displayed in the BD dashboard. The toggles live in users_meta — query with `listUserMeta database=subscription_types database_id=<the user's subscription_id>` once per task and read three keys: `show_profile_photo`, `show_logo_upload`, `coverPhoto` (note camelCase on cover, the other two are snake_case). All three are `"1"`/`"0"`. Routing for the headshot/icon image: both `show_profile_photo=1` and `show_logo_upload=1` → default to `profile_photo`. Only `show_logo_upload=1` → use `logo`. Only `show_profile_photo=1` → use `profile_photo`. Both `0` → don't import, tell user "this plan has profile photo and logo both disabled — enable on the plan or skip the image." Cover photo follows the same rule against `coverPhoto`: `1` → `cover_photo` is writable; `0` → don't import a cover. Writing to a disabled field is wasted — BD hides it on the dashboard and the value gets auto-cleared on the next save.
