...
`listWidgets`, `listForms`, `listFormFields`, `listSidebars`, `listMenus`, `listMenuItems` return a UNION of the site's own records and uncustomised platform master defaults. Filter `property=is_default property_value=false` for only the site's records, `true` for only master defaults — literal `false`/`true`; numeric forms are silently ignored. `is_default` is dependable as a filter on all six; its presence on returned rows varies by tool. `order_column`/`order_type` sort the merged result (e.g. add `order_column=revision_timestamp order_type=DESC limit=1` for the newest).
...
If a tool you need from this lookup is missing from your loaded `tools/list`, that is a session-config issue, NOT evidence the tool doesn't exist. Tell the user to verify their MCP client is loading the full BD catalog (170 tools); do not work around the absence.
...
**After every successful `createSingleImagePost` / `updateSingleImagePost` / `createMultiImagePost` / `updateMultiImagePost`, surface the BD admin edit URL to the user using exactly this pattern:** `https://ww2.managemydirectory.com/admin/viewPosts.php?search[value]=<post_id>&data_type=<data_type>&data_id=<data_id>&newsite=<website_id>`. Host is fixed; never substitute `BD_API_URL`. `post_id` from the create/update response. `data_type` and `data_id` from the post type's `listPostTypes` row (`data_type` = classification code like `20` for single-image; `data_id` = per-site post-type primary key — never hardcode). `website_id` from `getSiteInfo.message.website_id`. All four params required.
...
**Write responses are ALWAYS lean.** Every `create*` / `update*` across users, posts (single + multi), post types, top + sub categories, web pages, widgets, membership plans, email templates, forms + form fields, menus + menu items, redirects, and smart lists returns a minimal keep-set: primary key + identity fields (name / filename / title) + status. No nested schemas, no HTML body, no code templates, no embedded user object, no raw widget_data/widget_style/widget_javascript, no full `email_body`. Fields the agent explicitly passed in the request are always echoed back regardless of the keep set. The `include_*` flags do NOT apply to write responses — they only work on `get*` / `list*` / `search*`. If you need the full record after a write, call the matching `get*` with whatever `include_*` flags you actually need. Do NOT re-GET by default — the lean write echo is enough to confirm the write landed and to tell the user what changed. **Absent fields aren't unsupported** — the column exists, BD just didn't auto-fill it and you didn't set it. Tool schemas are the source of truth for what's settable.
...
**Swapping a photo at a specific slot.** BD's append does not reliably reuse the vacated `order` value; new photos can collide on `order=0` and shuffle the visible sequence. Pattern: (1) read the target's `order` via `listMultiImagePostPhotos`, (2) `deleteMultiImagePostPhoto photo_id=<old>`, (3) `updateMultiImagePost group_id=<same> post_image=<new URL> auto_image_import=1`, (4) re-list to find the new `photo_id` (its `original_image_url` matches the new URL), (5) `updateMultiImagePostPhoto photo_id=<new> order=<captured>`. Skip step 5 only when swapping the last slot.
...
**Plan signup/checkout URL.** Read `custom_checkout_url` via `getMembershipPlan` + `include_extras=1` — it's EAV-stored, so lean reads omit it and you'd wrongly fall back. Trim it, then:
- absolute URL (starts with `http`) → use verbatim (a site may point a plan at external checkout).
- slug/id → compose `getSiteInfo`'s `full_url` + `/` + `default_checkout_url` + `/` + the trimmed value.
- empty/whitespace/absent → same compose with `subscription_id` as the final segment.

`default_checkout_url` varies by site (`checkout`, `join`, `signup`, …) — use the returned value, never hardcoded. `full_url` has no trailing slash; single-`/` joins. E.g. `https://site.com/checkout/2`.
...
- **Orientation gate for Pexels-sourced feature image slots.** `post_image`, `hero_image`, and multi-image album photos require landscape — `searchStockImage` candidates satisfy this by construction (the API filters orientation and returns dimensions); otherwise verify via `getImageDimensions` per **Rule: Image dimensions**. Inline `<img>` in Froala body (`post_content`, `group_desc`) and email-template body (`email_body`) prefer landscape or square; portrait acceptable.
- **Pexels sourcing workflow.** Use this when an image is needed and **Rule: Image sourcing** routes to Pexels. **`searchStockImage` available → it replaces steps 1-4** (`query` = the 2-3 word topic phrase, `orientation=landscape`, `count=10`): pick the best-fitting candidate by description; steps 5-7 apply to its candidates unchanged. Pexels does NOT respect keyword orientation filters (no `wide`/`landscape`/`horizontal` keyword reliably surfaces landscape-only; those words search the photo's title/tags). `www.pexels.com/search/*` returns HTTP 403 to agent-side WebFetch on common runtimes (claude.ai); the WebSearch route below reaches Pexels via Google's index and is not gated.
  ...
  **User-supplied URLs (Pexels or other):** use as-is — user choice is the authority. **Batch submissions** (multi-image CSV): run the workflow for each slot; if a slot returns nothing usable, drop it and name it. Refuse the whole batch only if zero slots fill.
- **Format:** `.jpg`, `.jpeg`, or `.png`. WebP/GIF/AVIF candidates are skipped before any tool call per **Rule: Image dimensions**.
...
For Pexels candidate selection, retry strategy, and last-resort fallback, see **Rule: Image URLs** Pexels sourcing workflow.
...
- `leads` (via `createLead` / `updateLead`): any non-native field name auto-routes (compound identity `(database='leads', database_id=<lead_id>, key=<field_name>)`).
- `users_reviews` (via `createReview` / `updateReview`): any non-native field name auto-routes (compound identity `(database='users_reviews', database_id=<review_id>, key=<field_name>)`).
