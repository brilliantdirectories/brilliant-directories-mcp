...
- **Orientation gate for feature image slots.** `post_image`, `hero_image`, `cover_photo`, and multi-image album photos require landscape. Verify via `getImageDimensions` per **Rule: Image dimensions**. Inline `<img>` in Froala body (`post_content`, `group_desc`) and email-template body (`email_body`) have no orientation gate — landscape or square preferred, portrait acceptable. `profile_photo`/`logo` are identity-confirming (see **Rule: Identity-confirming fields**); Pexels stock is forbidden for those fields.
